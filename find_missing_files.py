...
'''
    ...
    def find_file_in_omnibase3(self, module_name: str) -> Path | None:
        ...
        possible_paths = [
            self.omnibase3_root / "src" / "omnibase" / "/".join(module_parts[:-1]) / f"{module_parts[-1]}.py",
            self.omnibase3_root / "src" / "omnibase" / "/".join(module_parts) / "__init__.py", 
        ...
        for py_file in python_files:
            ...
            for import_name in imports:
                ...
                try:
                    ...
                    expected_path = self.omnibase_core_root / "src" / "omnibase" / "/".join(import_parts[:-1]) / f"{import_parts[-1]}.py"
                    
                    if not expected_path.exists():
                        # Check if it exists as a package
                        expected_package = self.omnibase_core_root / "src" / "omnibase" / "/".join(import_parts) / "__init__.py"
                        ...
                        if not expected_package.exists():
                            ...
                            self.missing_files[import_name].referenced_by.append(str(py_file))
    ...
    def calculate_priority(self, import_name: str) -> int:
        ...
        critical_keywords = ['model_semver', 'model_contract', 'subcontract', 'core_errors', 'onex_error']
    ...
    def generate_report(self) -> None:
        ...
        for priority in [1, 2, 3]:
            if by_priority[priority]:
                ...
                for missing_file in sorted(by_priority[priority], key=lambda x: x.import_path):
                    print(f"📁 {missing_file.import_path}")
                    print(f"   Path: {missing_file.file_path}")
                    print(f"   In omnibase_3: {'✅' if missing_file.exists_in_omnibase3 else '❌'}")
                    ...
                    if len(missing_file.referenced_by) <= 3:
                        ...
                        for ref in missing_file.referenced_by[:2]:
                            print(f"     - {Path(ref).name}")
                        print(f"     ... and {len(missing_file.referenced_by) - 2} more")
                    print()
        
        # Summary statistics
        total_missing = len(self.missing_files)
        in_omnibase3 = sum(1 for f in self.missing_files.values() if f.exists_in_omnibase3)
        ...
        print(f"Success rate potential: {(in_omnibase3/total_missing)*100:.1f}%" if total_missing > 0 else "100%")
