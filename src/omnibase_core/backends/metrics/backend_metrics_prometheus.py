...
Usage:
    .. code-block:: python
        ...
        backend = BackendMetricsPrometheus(
            prefix="myapp",
            push_gateway_url="http://localhost:9091",  # url-authority-ok: local Prometheus example # onex-allow-internal-ip OMN-17524 reason="documentation example"
...
def sanitize_url(url: str | None) -> str:
    ...
    Example:
        ...
        >>> example_url = "http://localhost:9091"  # onex-allow-internal-ip OMN-17524 reason="sanitizer doctest"
        >>> sanitize_url(example_url) == example_url
        True
        >>> sanitize_url(None)
        '<no-url>'
