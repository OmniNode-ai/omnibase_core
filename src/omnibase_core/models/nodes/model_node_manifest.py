"""
Node Manifest Model - Tier 2 Metadata

Pydantic model for node-level metadata in the three-tier metadata system.
Represents individual nodes within a group with version management and capabilities.
"""
...
class ModelNodeDependency(BaseModel):
    """Node-specific dependency definition."""
...
class ModelNodeIntegration(BaseModel):
    """Service integration configuration for node."""

    auto_load_strategy: str = Field(
        default="current_stable",
        description="Strategy for loading node versions",
    )
    ...
    requires_separate_port: bool = Field(
        default=False,
        description="Whether node requires separate HTTP port",
    )
    initialization_order: int = Field(
        default=5,
        description="Initialization order relative to other nodes",
    )
...
class ModelNodeTesting(BaseModel):
...
class ModelNodeSecurity(BaseModel):
    """Security configuration and requirements."""

    processes_sensitive_data: bool = Field(
        default=False,
        description="Whether node processes sensitive data",
    )
    ...
    requires_network_access: bool = Field(
        default=False,
        description="Whether node requires network access",
    )
...
class ModelNodeManifest(BaseModel):
    """
    Tier 2: Node-level metadata model.

    Defines node-specific configuration, version catalog, dependencies,
    capabilities, and integration requirements.
    """

    # === NODE IDENTITY ===
    node_name: str = Field(description="Unique node identifier within group")
    description: str = Field(description="Node purpose and functionality")
    author: str = Field(default="ONEX Framework Team", description="Node author")
    created_date: datetime = Field(description="Node creation timestamp")
    ...
    meta_type: str = Field(default="node", description="Meta-type classification")
    ...
    # === NODE STATUS ===
    status: EnumNodeStatus = Field(description="Current node lifecycle status")
    lifecycle: str = Field(default="active", description="Node lifecycle state")
    ...
    versions: list[ModelNodeVersion] = Field(
        description="Available versions with lifecycle information",
    )

    # === CAPABILITIES ===
    capabilities: list[ModelNodeCapability] = Field(
    ...
    # === DEPENDENCIES ===
    dependencies: list[ModelNodeDependency] = Field(
        default_factory=list,
        description="Node-specific dependencies",
    )

    # === SERVICE INTEGRATION ===
    integration: ModelNodeIntegration = Field(
    ...
    testing: ModelNodeTesting = Field(
        description="Testing requirements and configuration",
    )

    # === SECURITY CONFIGURATION ===
    security: ModelNodeSecurity = Field(
    ...
    schema_version: SemVerField = Field(
        ...
        description="Node manifest schema version",
    )
    uuid: str | None = Field(
        default=None,
    ...
    def validate_versions_list(
        cls,
        v: list[ModelNodeVersion],
    ) -> list[ModelNodeVersion]:
    ...
    @classmethod
    def validate_testing_config(cls, v: ModelNodeTesting) -> ModelNodeTesting:
    ...
    def get_version_by_string(self, version_string: str) -> ModelNodeVersion | None:
    ...
    def get_active_versions(self) -> list[ModelNodeVersion]:
    ...
    def get_recommended_version(self) -> ModelNodeVersion | None:
