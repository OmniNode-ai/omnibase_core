...
class ModelRegistryOperationalSummary(BaseModel):
    ...
    external_services_count: int = Field(
        0,
        description="Number of external services configured",
        ge=0,
    )
    ...
    estimated_time_seconds: int = Field(
        ...,
        description="Estimated resolution time in seconds",
        ge=1,
    )
    ...
    average_resolution_time_ms: float | None = Field(
        None,
        description="Average resolution time in milliseconds",
    )

    total_resolutions: int | None = Field(
        None,
        description="Total number of resolutions performed",
        ge=0,
    )
