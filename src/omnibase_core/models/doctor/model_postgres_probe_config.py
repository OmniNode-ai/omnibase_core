    ...
    def from_overlay(cls) -> ModelPostgresProbeConfig:
        ...
        if not host:
            raise ModelOnexError(
                message=(
                    "POSTGRES_HOST is not bound by the active overlay; declare "
                    "it in the per-lane overlay so the contract reference "
                    f"{_HOST_CONTRACT_REF!r} resolves to a Postgres host."
                ),
                error_code=EnumCoreErrorCode.INVALID_CONFIGURATION,
                context={"contract_ref": _HOST_CONTRACT_REF, "field": "host"},
            )
        port_str = expand_contract_env_refs(_PORT_CONTRACT_REF)
        if not port_str:
            raise ModelOnexError(
                message=(
                    "POSTGRES_PORT is not bound by the active overlay; declare "
                    "it in the per-lane overlay so the contract reference "
                    f"{_PORT_CONTRACT_REF!r} resolves to a Postgres port."
                ),
                error_code=EnumCoreErrorCode.INVALID_CONFIGURATION,
                context={"contract_ref": _PORT_CONTRACT_REF, "field": "port"},
            )
        try:
            port = int(port_str)
            ...
            raise ModelOnexError(
                message=(
                    f"POSTGRES_PORT overlay value {port_str!r} is not a valid "
                    "port integer."
                ),
                error_code=EnumCoreErrorCode.INVALID_CONFIGURATION,
                context={"contract_ref": _PORT_CONTRACT_REF, "value": port_str},
            ) from exc
