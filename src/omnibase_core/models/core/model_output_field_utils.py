...
def make_output_field(field_value: Any, output_field_model_cls: type[T]) -> T:
    """
    Converts/wraps any field_value (dict[str, Any], Pydantic model, etc.) into the canonical output field model.
    Args:
        field_value: The value to wrap (dict[str, Any], model, etc.)
...
def build_output_field_kwargs(
    input_state: dict[str, Any], event_bus: Any
) -> dict[str, Any]:
    """
    Build canonical output_field kwargs for a node, handling backend, custom, integration, processed keys.
    Args:
        input_state: The input state dict[str, Any]event_bus: The event bus instance (for backend name)
    Returns:
        dict[str, Any]of kwargs for output field model
    """
...
class ModelComputeOutputFieldTool(OutputFieldTool):
    def __call__(self, state, input_state_dict: dict[str, Any]) -> ModelOnexField:
        ...
        if val is not None:
            ...
            return ModelOnexField(data=val)
        # If 'external_dependency' is present and True in either model or dict[str, Any], return integration
        if (
