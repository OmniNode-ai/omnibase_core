"""
Metadata node usage metrics model.
"""

from datetime import datetime

from pydantic import BaseModel, Field


class ModelMetadataNodeUsageMetrics(BaseModel):
    """Usage and performance metrics for metadata nodes."""

    total_invocations: int = Field(0, description="Total number of node invocations")
    success_count: int = Field(0, description="Number of successful invocations")
    failure_count: int = Field(0, description="Number of failed invocations")
    avg_processing_time_ms: float = Field(
        0.0,
        description="Average processing time in milliseconds",
    )
    last_used: datetime | None = Field(None, description="Last usage timestamp")
    most_recent_error: str | None = Field(
        None,
        description="Most recent error message",
    )
    popularity_score: float = Field(
        0.0,
        description="Popularity score based on usage (0-100)",
    )
