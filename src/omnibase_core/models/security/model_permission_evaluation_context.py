...
class ModelPermissionEvaluationContext(BaseModel):
    ...
    def get(
        ...
        """Get a value from context attributes (dict-like behavior)."""
        ...
        # Check typed attribute dictionaries
    ...
    def __contains__(self, key: str) -> bool:
        """Check if key exists in context (dict-like behavior)."""
    ...
    def __getitem__(self, key: str) -> str | int | bool:
        """Get item with indexing (dict-like behavior)."""
        value = self.get(key)
        if value is None:
