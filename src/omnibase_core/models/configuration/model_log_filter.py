...
class ModelLogFilterConfig(BaseModel):
    ...
    regex_timeout_ms: int | None = Field(
        None,
        description="Regex match timeout in milliseconds",
        ge=100,
        le=10000,
    )
    ...
    field_type_validation: bool = Field(
        True,
        description="Validate field types before comparison",
    )
    ...
    redact_matched_content: bool = Field(
        False,
        description="Redact matched content in output",
    )
    redaction_pattern: str = Field(
        "[REDACTED]",
        description="Pattern to use for redaction",
    )

    # Custom script support
    custom_script_path: str | None = Field(
        None,
        description="Path to custom filter script",
    )
    custom_script_timeout_ms: int | None = Field(
        None,
        description="Custom script timeout in milliseconds",
        ge=100,
    )
...
class ModelLogFilter(BaseModel):
    ...
    def _matches_level_range(self, level_value: int) -> bool:
        ...
        return not (self.max_level is not None and level_value > self.max_level)
