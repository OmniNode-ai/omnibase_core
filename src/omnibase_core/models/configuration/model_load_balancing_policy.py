...
class ModelLoadBalancingPolicy(BaseModel):
    ...
    def create_round_robin_policy(
        ...
        return cls(
            ...
            display_name="Round Robin Policy",
            description="Simple round-robin load balancing with health checks",
            algorithm=ModelLoadBalancingAlgorithm.create_round_robin(),
            health_check=ModelHealthCheckConfig.create_production_checks(),
            circuit_breaker=ModelCircuitBreaker.create_fast_fail(),
        )
    ...
    def create_weighted_policy(
        ...
        return cls(
            policy_name=policy_name,
            display_name="Weighted Round Robin Policy",
            description="Weighted round-robin load balancing with health checks",
            algorithm=ModelLoadBalancingAlgorithm.create_weighted_round_robin(),
            node_weights=ModelNodeWeights.create_custom_weights(node_weights),
            health_check=ModelHealthCheckConfig.create_production_checks(),
    ...
    def create_least_connections_policy(
        ...
        return cls(
            policy_name=policy_name,
            display_name="Least Connections Policy",
            description="Least connections load balancing with circuit breaker",
            algorithm=ModelLoadBalancingAlgorithm.create_least_connections(),
            health_check=ModelHealthCheckConfig.create_fast_checks(),
            circuit_breaker=ModelCircuitBreaker.create_fast_fail(),
        )
    ...
    def create_sticky_session_policy(
        ...
        return cls(
            policy_name=policy_name,
            display_name="Sticky Sessions Policy",
            description="IP hash load balancing with cookie-based session affinity",
            algorithm=ModelLoadBalancingAlgorithm.create_ip_hash(),
            session_affinity=ModelSessionAffinity.create_cookie_affinity(cookie_name),
            health_check=ModelHealthCheckConfig.create_production_checks(),
            circuit_breaker=ModelCircuitBreaker.create_resilient(),
        )
    ...
    def create_high_availability_policy(
        ...
        return cls(
            policy_name=policy_name,
            display_name="High Availability Policy",
            description="Comprehensive fault-tolerant load balancing with all features enabled",
            algorithm=ModelLoadBalancingAlgorithm.create_resource_based(),
            health_check=ModelHealthCheckConfig.create_thorough_checks(),
            circuit_breaker=ModelCircuitBreaker.create_resilient(),
            retry_policy=ModelRetryPolicy(max_attempts=5, initial_delay_ms=500),
