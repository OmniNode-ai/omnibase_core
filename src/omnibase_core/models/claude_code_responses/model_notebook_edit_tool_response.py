...
class ModelNotebookEditToolResponse(BaseModel):
    ...
    backup_created: bool = Field(
        default=False,
        description="Whether backup was created before edit",
    )
    backup_path: str | None = Field(
        None,
        description="Path to backup file if created",
    )
    notebook_version: str | None = Field(
        None,
        description="Jupyter notebook format version",
    )
