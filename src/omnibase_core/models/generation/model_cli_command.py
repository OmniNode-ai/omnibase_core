...
class ModelCliCommand(BaseModel):
    ...
    def from_dict(cls, data: dict[str, Any]) -> "ModelCliCommand":
        ...
        # Handle both "name" and "command_name" fields for current standards
        ...
        for option_data in data.get("options", []):
            if isinstance(option_data, str):
                ...
                # Convert dict to ModelCliOptionData first
                from omnibase_core.models.generation.model_cli_option import (
                    ModelCliOptionData,
                )

                cli_option_data = ModelCliOptionData(**option_data)
                options.append(ModelCliOption.from_dict(cli_option_data))
