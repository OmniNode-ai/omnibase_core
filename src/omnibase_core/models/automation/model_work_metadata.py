...
class ModelWorkMetadata(BaseModel):
    ...
    similar_work_count: int = Field(
        default=0,
        description="Number of similar work items found",
    )
    historical_success_rate: float = Field(
        default=0.0,
        description="Success rate of similar work",
    )
    recommended_agent_tier: str = Field(
        default="standard",
        description="Recommended agent performance tier",
    )
    safety_checks_passed: int = Field(
        default=0,
        description="Number of safety checks passed",
    )
    safety_checks_failed: int = Field(
        default=0,
        description="Number of safety checks failed",
    )
