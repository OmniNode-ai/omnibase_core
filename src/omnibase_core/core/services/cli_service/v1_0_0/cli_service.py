...
class ProtocolTool(Protocol):
    ...
    def health_check(self) -> Union[Dict[str, Union[str, bool]], bool, None]:
    ...
    def get_tool_metadata(self) -> Dict[str, Union[str, int, bool]]:
        """Optional metadata method."""
        ...


ToolInputStateType = Union[Dict[str, Union[str, int, bool]], object]
ToolResultType = Union[Dict[str, Union[str, int, bool]], object, str, int, bool, None]
...
class CliService:
    ...
    def __init__(self, config: Optional[ModelCliConfig] = None):
        """
        ...
        try:
            ...
            while i < len(args):
                ...
                if arg.startswith("--"):
                    ...
                    # Positional argument
                    pos_key = f"arg_{len([k for k in parsed_args.keys() if k.startswith('arg_')])}"
    ...
    def handle_introspect_flag(
        ...
        try:
            ...
            if hasattr(tool_instance, "get_introspection_data"):
                try:
                    ...
                    emit_log_event(
                        ...
                        f"Failed to get comprehensive introspection data: {str(e)}, using fallback",
            ...
            if self._config.output_format == "json":
                ...
                formatted_data: Dict[str, Union[str, int, bool, List[str]]] = {}
    ...
    def convert_args_to_input_state(
        self, parsed_args: Dict[str, Union[str, int, bool]], tool_instance: ProtocolTool
    ) -> ToolInputStateType:
        """
        ...
        try:
            ...
            if hasattr(tool_instance, "_get_input_state_class"):
                ...
                return result_obj
            elif hasattr(tool_instance, "input_state_class"):
            ...
            emit_log_event(
                LogLevelEnum.ERROR,
                f"Failed to convert CLI args to input state: {str(e)}",
                {
                    "tool_type": type(tool_instance).__name__,
            ...
            raise OnexError(
                ...
                message=f"Failed to convert CLI arguments to input state: {str(e)}",
        ...
        try:
            ...
            if contract_path and contract_path.exists():
                try:
                    ...
                    emit_log_event(
                        LogLevelEnum.WARNING,
                        f"Failed to parse contract CLI interface: {str(e)}",
                        {"contract_path": str(contract_path)},
                    )
    ...
    def handle_cli_error(self, error: Exception, operation: str) -> ModelCliResult:
        ...
        emit_log_event(
            LogLevelEnum.ERROR,
            f"CLI error in {operation}: {str(error)}",
            {
                "operation": operation,
        ...
        return ModelCliResult(
            ...
            success=False,
            message=f"CLI error in {operation}: {str(error)}",
            should_exit=True,
            error_details=error_details_model,
        )

    def determine_exit_code(
        self, result: ToolResultType, error: Optional[Exception] = None
    ) -> int:
        """
        ...
        if hasattr(result, "status"):
            ...
            elif status in [EnumOnexStatus.ERROR.value, EnumOnexStatus.UNKNOWN.value]:
                return int(self._config.default_exit_code_error)
            else:
    ...
    def _format_introspection_text(
        self, data: Dict[str, Union[str, int, bool, List[str]]]
    ) -> str:
        """
