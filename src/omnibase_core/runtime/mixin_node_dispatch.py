...
from omnibase_core.types.type_node_dispatch import DispatcherCallable, DlqTopicDeriver
...
class MixinNodeDispatch:
    ...
    def _payload_matches(entry: DispatchEntry, payload: object | None) -> bool:
        ...
        try:
            ...
            # fallback-ok: an injected matcher is a selection-isolation boundary;
            # any failure means non-match and must not abort dispatcher selection.
            return False
    ...
    async def dispatch(
        ...
        * ``SUCCESS`` with the ordered matched dispatcher ids otherwise.
        ...
        return ModelDispatchResult(
            ...
            dispatcher_id=DispatcherId(", ".join(ordered_ids)),
            topic=topic,
            message_category=topic_category,
            message_type=message_type,
            started_at=started_at,
    ...
    def _derive_dlq_topic(self, event_type: str | None, topic: str) -> str | None:
        ...
        try:
            ...
            # fallback-ok: injected DLQ derivation supplies optional selection
            # metadata; failure preserves NO_DISPATCHER and must not abort selection.
            return None
