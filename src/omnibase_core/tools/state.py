...
class ModelSplitterConfig(BaseModel):
    ...
    create_backups: bool = Field(
        default=True, description="Whether to create backups before modifying files"
    )
    update_imports: bool = Field(
        default=True, description="Whether to update imports across the codebase"
    )
    maintain_compatibility: bool = Field(
        default=True, description="Whether to maintain backward compatibility imports"
    )
    split_mode: EnumSplitMode = Field(
        default=EnumSplitMode.ONE_PER_FILE, description="Mode for splitting models"
    )
    preserve_comments: bool = Field(
        default=True, description="Whether to preserve comments and docstrings"
    )
    output_file_prefix: str = Field(
        default="model_", description="Prefix for generated model files"
    )
...
class ModelInfo(BaseModel):
    ...
    docstring: Optional[str] = Field(
        default=None, description="Model docstring if present"
    )
    decorators: List[str] = Field(
        default_factory=list, description="Decorators applied to the model"
    )
...
class ModelExtractionPlan(BaseModel):
    ...
    compatibility_imports: Dict[str, List[str]] = Field(
        default_factory=dict, description="Imports needed for backward compatibility"
    )
...
class ModelSplitterResult(BaseModel):
    ...
    extraction_plan: Optional[ModelExtractionPlan] = Field(
        default=None, description="Plan for extracting models"
    )
    ...
    import_updates: Dict[str, List[str]] = Field(
        default_factory=dict, description="Import updates needed across codebase"
    )
...
class ModelSplitterInputState(OnexInputState):
    ...
    output_dir: Optional[str] = Field(
        default=None, description="Directory where split model files will be created"
    )
...
class ModelSplitterOutputState(OnexOutputState):
    ...
    operation_result: Optional[ModelSplitterResult] = Field(
        default=None, description="Complete operation results"
    )
