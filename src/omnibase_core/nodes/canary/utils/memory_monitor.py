...
class MemoryMonitor:
    ...
    def _get_memory_metrics(self, operation_name: Optional[str] = None) -> MemoryMetrics:
    ...
    def check_memory_usage(
        ...
        try:
            ...
            if self._baseline_metrics:
                growth_mb = current_metrics.process_memory_mb - self._baseline_metrics.process_memory_mb
    ...
    async def cleanup(self) -> None:
        ...
        if tasks_to_cancel:
            ...
            self.logger.debug(f"Cancelled {len(tasks_to_cancel)} memory monitoring tasks")
...
async def memory_monitored_operation(
    ...
    try:
        ...
        if monitor._baseline_metrics:
            total_growth = final_metrics.process_memory_mb - monitor._baseline_metrics.process_memory_mb
            monitor.logger.info(
