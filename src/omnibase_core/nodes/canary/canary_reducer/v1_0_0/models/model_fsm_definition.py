...
class ModelFSMDefinition(BaseModel):
    ...
    @classmethod
    def validate_states_contain_initial(cls, v, info):
        """Validate that states list contains the initial state."""
        if hasattr(info.data, "get") and "initial_state" in info.data:
    ...
    @classmethod
    def validate_special_states_exist(cls, v, info):
        """Validate that terminal and error states exist in the states list."""
        if hasattr(info.data, "get") and "states" in info.data and v:
    ...
    @classmethod
    def validate_transition_states_exist(cls, v, info):
        """Validate that all transition from/to states exist."""
        if hasattr(info.data, "get") and "states" in info.data:
            state_names = {state.state_name for state in info.data["states"]}
            for transition in v:
