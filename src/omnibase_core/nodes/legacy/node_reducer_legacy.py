...
NodeReducerLegacy - Data Aggregation Node for 4-Node Architecture (DEPRECATED).

.. deprecated:: 0.4.0
    NodeReducerLegacy is deprecated. Use NodeReducer (the declarative implementation,
    formerly NodeReducerDeclarative) instead. See docs/guides/MIGRATING_TO_DECLARATIVE_NODES.md
...
class NodeReducerLegacy(NodeCoreBase):
    """
    .. deprecated:: 0.4.0
        NodeReducerLegacy is deprecated. Use NodeReducer (the declarative implementation,
        formerly NodeReducerDeclarative) instead. See docs/guides/MIGRATING_TO_DECLARATIVE_NODES.md
    ...
    def __init__(self, container: ModelONEXContainer) -> None:
        """
        Initialize NodeReducerLegacy with ModelONEXContainer dependency injection.
    ...
    async def _initialize_node_resources(self) -> None:
        ...
        if config:
            ...
            emit_log_event(
                LogLevel.INFO,
                "NodeReducerLegacy loaded configuration from NodeConfigProvider",
                {
                    "node_id": str(self.node_id),
        ...
        emit_log_event(
            LogLevel.INFO,
            "NodeReducerLegacy resources initialized",
            {
                "node_id": str(self.node_id),
                "default_batch_size": self.default_batch_size,
                "max_memory_usage_mb": self.max_memory_usage_mb,
    ...
    async def _cleanup_node_resources(self) -> None:
        ...
        emit_log_event(
            LogLevel.INFO,
            "NodeReducerLegacy resources cleaned up",
            {"node_id": str(self.node_id)},
        )
