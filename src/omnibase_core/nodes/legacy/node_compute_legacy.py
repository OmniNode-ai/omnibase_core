...
.. deprecated:: 0.4.0
    NodeComputeLegacy is deprecated. For new implementations, use compute contracts
    with NodeCoreBase directly. See docs/guides/MIGRATING_TO_DECLARATIVE_NODES.md

LEGACY - STABLE INTERFACE v1.0.0

NodeComputeLegacy - Pure Computation Node for 4-Node Architecture.
...
class NodeComputeLegacy(NodeCoreBase):
    """
    .. deprecated:: 0.4.0
        NodeComputeLegacy is deprecated. For new implementations, use compute contracts
        with NodeCoreBase directly. See docs/guides/MIGRATING_TO_DECLARATIVE_NODES.md
    ...
    def __init__(self, container: ModelONEXContainer) -> None:
        """
        Initialize NodeComputeLegacy with ModelONEXContainer dependency injection.
    ...
    async def _initialize_node_resources(self) -> None:
        ...
        if config:
            ...
            emit_log_event(
                LogLevel.INFO,
                "NodeComputeLegacy loaded configuration from NodeConfigProvider",
                {
                    "node_id": str(self.node_id),
        ...
        emit_log_event(
            LogLevel.INFO,
            "NodeComputeLegacy resources initialized",
            {
                "node_id": str(self.node_id),
                "max_parallel_workers": self.max_parallel_workers,
                "cache_ttl_minutes": self.cache_ttl_minutes,
    ...
    async def _cleanup_node_resources(self) -> None:
        ...
        emit_log_event(
            LogLevel.INFO,
            "NodeComputeLegacy resources cleaned up",
            {"node_id": str(self.node_id)},
        )
