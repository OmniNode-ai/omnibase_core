...
NodeOrchestratorLegacy - Workflow Coordination Node for 4-Node Architecture.
...
# Unified error message constants for workflow definition errors
_ERR_WORKFLOW_ID_EMPTY = "Workflow definition error: workflow ID cannot be empty"
_ERR_WORKFLOW_NO_STEPS = (
    "Workflow definition error: workflow must have at least one step"
)
_ERR_WORKFLOW_INVALID_MODE = (
    "Workflow definition error: execution mode must be valid EnumExecutionMode enum"
)
_ERR_WORKFLOW_HAS_CYCLES = (
    "Workflow definition error: workflow contains dependency cycles"
)
_ERR_CONDITION_ALREADY_REGISTERED = (
    "Workflow definition error: condition function already registered"
)
_ERR_CONDITION_NOT_CALLABLE = (
    "Workflow definition error: condition function must be callable"
)


class NodeOrchestratorLegacy(NodeCoreBase):
    """
    .. deprecated:: 0.4.0
        NodeOrchestratorLegacy is deprecated. Use NodeOrchestrator (the declarative implementation,
        formerly NodeOrchestratorDeclarative) instead. See docs/guides/MIGRATING_TO_DECLARATIVE_NODES.md

    LEGACY - STABLE INTERFACE v1.0.0
    ...
    condition_functions: dict[
        str, Callable[[ModelWorkflowStepExecution, list[Any]], bool]
    ]

    def __init__(self, container: ModelONEXContainer) -> None:
        """
        Initialize NodeOrchestratorLegacy with ModelONEXContainer dependency injection.
        ...
        warnings.warn(
            "NodeOrchestratorLegacy is deprecated since v0.4.0. "
            "Use NodeOrchestrator from omnibase_core.nodes instead. "
                ...
                if input_data.dependency_resolution_enabled:
                    ...
                    if dependency_graph.has_cycles():
                        raise ModelOnexError(
                            ...
                            message=_ERR_WORKFLOW_HAS_CYCLES,
                            context={
    ...
    def register_condition_function(
        self,
        condition_name: str,
        function: Callable[[ModelWorkflowStepExecution, list[Any]], bool],
    ) -> None:
        """
        ...
        Args:
            ...
            function: Function that takes (step, previous_results) and returns bool
        ...
        if condition_name in self.condition_functions:
            raise ModelOnexError(
                error_code=EnumCoreErrorCode.VALIDATION_ERROR,
                message=f"{_ERR_CONDITION_ALREADY_REGISTERED}: {condition_name}",
                context={
                    "node_id": str(self.node_id),
        ...
        if not callable(function):
            raise ModelOnexError(
                error_code=EnumCoreErrorCode.VALIDATION_ERROR,
                message=f"{_ERR_CONDITION_NOT_CALLABLE}: {condition_name}",
                context={
                    "node_id": str(self.node_id),
                    "condition_name": condition_name,
                },
    ...
    async def _initialize_node_resources(self) -> None:
        ...
        if config:
            ...
            emit_log_event(
                LogLevel.INFO,
                "NodeOrchestratorLegacy loaded configuration from NodeConfigProvider",
                {
                    "node_id": str(self.node_id),
        ...
        emit_log_event(
            LogLevel.INFO,
            "NodeOrchestratorLegacy resources initialized",
            {
                "node_id": str(self.node_id),
                "max_concurrent_workflows": self.max_concurrent_workflows,
                "action_emission_enabled": self.action_emission_enabled,
    ...
    async def _cleanup_node_resources(self) -> None:
        ...
        emit_log_event(
            LogLevel.INFO,
            "NodeOrchestratorLegacy resources cleaned up",
            {"node_id": str(self.node_id)},
        )
    ...
    def _validate_orchestrator_input(self, input_data: ModelOrchestratorInput) -> None:
        ...
        if not input_data.workflow_id:
            raise ModelOnexError(
                error_code=EnumCoreErrorCode.VALIDATION_ERROR,
                message=_ERR_WORKFLOW_ID_EMPTY,
                context={
                    "node_id": str(self.node_id),
                    "operation_id": str(input_data.operation_id),
                },
        ...
        if not input_data.steps:
            raise ModelOnexError(
                error_code=EnumCoreErrorCode.VALIDATION_ERROR,
                message=_ERR_WORKFLOW_NO_STEPS,
                context={
                    "node_id": str(self.node_id),
                    "workflow_id": str(input_data.workflow_id),
                },
        ...
        if not isinstance(input_data.execution_mode, EnumExecutionMode):
            raise ModelOnexError(
                error_code=EnumCoreErrorCode.VALIDATION_ERROR,
                message=_ERR_WORKFLOW_INVALID_MODE,
                context={
                    "node_id": str(self.node_id),
    ...
    async def _execute_sequential_workflow(
        ...
        for step in ordered_steps:
            try:
                ...
                # Execute step and emit actions
                step_results, step_actions = await self._execute_step_and_emit_actions(
                    step, input_data.workflow_id
                )
        ...
        if dependency_graph:
            ...
            while True:
                ...
                for batch_start in range(0, len(ready_steps), max_parallel):
                    ...
                    # Wait for all tasks to complete using gather for better parallelism
                    results = await asyncio.gather(*tasks, return_exceptions=True)

                    # Process results from parallel execution
                    for step, result in zip(batch_steps, results, strict=True):
                        if isinstance(result, BaseException):
                            step.state = EnumWorkflowState.FAILED
                            # Cast to Exception for the model's type constraint
                            step.error = (
                                result
                            ...
                            # result is list[Any] at this point
                            success_result: list[Any] = result
                            step.state = EnumWorkflowState.COMPLETED
                            completed_step_ids.append(str(step.step_id))
                            all_results.extend(success_result)
                            dependency_graph.mark_completed(step.step_id)

                            # Emit actions for completed step
                            _, step_actions = await self._execute_step_and_emit_actions(
                                step, input_data.workflow_id, execute=False
                            )
                            all_actions.extend(step_actions)
            ...
            for batch_start in range(0, len(workflow_steps), max_parallel):
                ...
                for step, result in zip(batch_steps, results, strict=True):
                    if isinstance(result, BaseException):
                        ...
                        # result is list[Any] at this point
                        success_result_no_deps: list[Any] = result
                        step.state = EnumWorkflowState.COMPLETED
                        completed_step_ids.append(str(step.step_id))
                        all_results.extend(success_result_no_deps)
    ...
    async def _execute_batch_workflow(
        ...
        FIX (PR #113): This method now executes steps exactly once using the load
        balancer for operation distribution. Previously, there was a bug where
        steps could be executed twice - once in the batch loop and again via
        a separate call to _execute_sequential_workflow.

        The fix ensures:
        1. Each step is executed exactly once via _execute_single_step
        2. Actions are emitted via _execute_step_and_emit_actions with execute=False
        3. Load balancer coordinates resource acquisition/release
        """
        completed_step_ids: list[str] = []
        failed_step_ids: list[str] = []
        all_actions: list[ModelAction] = []
        ...
        for _group_type, steps in step_groups.items():
            for step in steps:
                ...
                if operation_acquired:
                    ...
                    try:
                        # Execute step ONCE via _execute_single_step
                        step_result = await self._execute_single_step(step)

                        # Emit actions for this step (execute=False to avoid double execution)
                        _, step_actions = await self._execute_step_and_emit_actions(
                            step, input_data.workflow_id, execute=False
                        )
                        all_actions.extend(step_actions)
                        ...
                        # Update dependency graph if present
                        if dependency_graph:
                            dependency_graph.mark_completed(step.step_id)
        ...
        return ModelOrchestratorOutput(
            ...
            actions_emitted=all_actions,
            metrics={
                "load_balanced_operations": float(load_balanced_operations),
                "execution_mode_batch": 1.0,
            },
        )

    async def _execute_step_and_emit_actions(
        self,
        step: ModelWorkflowStepExecution,
        workflow_id: UUID,
        execute: bool = True,
    ) -> tuple[list[Any], list[ModelAction]]:
        """
        Execute step actions and emit them.

        This method consolidates the duplicate action emission logic.

        Args:
            step: The workflow step to execute
            workflow_id: The workflow ID for context
            execute: Whether to actually execute the step (vs just emit actions)

        Returns:
            Tuple of (step_results, emitted_actions)
        """
        step_results: list[Any] = []
        emitted_actions: list[ModelAction] = []
        ...
        for action in step.thunks:
            if self.action_emission_enabled:
                ...
                emitted_actions.append(emitted_action)
        ...
        return step_results, emitted_actions
    ...
    async def _execute_single_step(
        ...
        FIX (PR #113): Removed unused workflow_id parameter.
        Actions are emitted separately via _execute_step_and_emit_actions.

        Args:
            step: The workflow step to execute

        Returns:
            List of execution results for each action in the step
        """
        step.state = EnumWorkflowState.RUNNING
        step.started_at = datetime.now()
    ...
    async def _evaluate_condition(
        ...
        FIX (PR #113): All condition evaluations now explicitly return bool type.
        Custom condition functions are wrapped with bool() to ensure type consistency.

        Args:
            step: The workflow step with condition to evaluate
    ...
    def _get_topological_order(
        ...
        # Kahn's algorithm using deque for O(1) queue operations
    ...
    def _group_steps_by_type(
        self, workflow_steps: list[ModelWorkflowStepExecution]
    ) -> dict[str, list[ModelWorkflowStepExecution]]:
        """Group workflow steps by their action types for batch processing."""
        groups: dict[str, list[ModelWorkflowStepExecution]] = {}

        for step in workflow_steps:
            # Determine group key from action types in the step
            if step.thunks:
                action_types = [action.action_type.value for action in step.thunks]
                group_key = "_".join(sorted(set(action_types)))
            else:
                group_key = "empty"
    ...
    def _register_builtin_conditions(self) -> None:
        ...
        FIX (PR #113): All condition functions now have explicit bool return type
        annotations and return bool values consistently.

        All condition functions have the signature:
            (step: ModelWorkflowStepExecution, previous_results: list[Any]) -> bool
        """

        def always_true(
            _step: ModelWorkflowStepExecution, _previous_results: list[Any]
        ) -> bool:
            """Always returns True - unconditional execution."""
            return True

        def always_false(
            _step: ModelWorkflowStepExecution, _previous_results: list[Any]
        ) -> bool:
            """Always returns False - skip execution."""
            return False

        def has_previous_results(
            _step: ModelWorkflowStepExecution,
            previous_results: list[Any],
        ) -> bool:
            """Returns True if there are any previous results."""
        ...
        def previous_step_success(
            ...
            if isinstance(last_result, dict):
                return last_result.get("status") == "executed"
            return False

        # Register conditions with explicit bool return types
