...
class NodeOrchestratorLegacy(NodeCoreBase):
    ...
    async def _execute_batch_workflow(
        ...
        FIX: Previously this method was executing steps twice - once in the loop
        and again via _execute_sequential_workflow. Now it executes steps only once
        using the load balancer for operation distribution.
        """
        completed_step_ids: list[str] = []
        failed_step_ids: list[str] = []
        all_actions: list[ModelAction] = []
    ...
    async def _execute_single_step(
        ...
        Note: workflow_id parameter removed as it was unused.
    ...
    def _detect_dependency_cycles(
        ...
        """
        Detect all dependency cycles in workflow.
        ...
        Returns:
            List of cycles, where each cycle is a list of step IDs forming the cycle
        """
        ...
        def dfs(node: UUID) -> None:
            ...
            for neighbor in edges.get(node, []):
                if neighbor not in visited:
                    ...
                    # Found cycle - extract it using list unpacking
                    cycle_start = path.index(neighbor)
                    cycle = [str(n) for n in [*path[cycle_start:], neighbor]]
                    cycles.append(cycle)
