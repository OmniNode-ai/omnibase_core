"""
NodeEffectLegacy - Legacy code-driven effect node.

.. deprecated:: 0.4.0
    Use :class:`omnibase_core.nodes.NodeEffect` (contract-driven) instead.
    This legacy implementation is maintained for backward compatibility with
    existing code that uses effect_handlers and the code-driven pattern.

VERSION: 1.0.0 (Legacy)
STABILITY GUARANTEE: Abstract method signatures frozen.

Migration Guide:
    The new contract-driven NodeEffect uses YAML contracts for effect definitions.
    See docs/architecture/CONTRACT_DRIVEN_NODEEFFECT_V1_0.md for migration steps.
...
class NodeEffectLegacy(NodeCoreBase):
    """
    Legacy code-driven effect node.

    .. deprecated:: 0.4.0
        Use :class:`omnibase_core.nodes.NodeEffect` (contract-driven) instead.

    Side effect management node for external interactions using code-driven handlers.
