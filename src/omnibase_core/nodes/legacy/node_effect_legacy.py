...
NodeEffectLegacy - Side Effect Management Node for 4-Node Architecture.
...
class NodeEffectLegacy(NodeCoreBase):
    """
    .. deprecated:: 0.4.0
        NodeEffectLegacy is deprecated. For new implementations, use effect contracts
    ...
    def __init__(
        ...
        """
        Initialize NodeEffectLegacy with ModelONEXContainer dependency injection.
    ...
    async def _initialize_node_resources(self) -> None:
        ...
        if config:
            ...
            emit_log_event(
                LogLevel.INFO,
                "NodeEffectLegacy loaded configuration from NodeConfigProvider",
                {
                    "node_id": str(self.node_id),
        ...
        emit_log_event(
            LogLevel.INFO,
            "NodeEffectLegacy resources initialized",
            {
                "node_id": str(self.node_id),
                "max_concurrent_effects": self.max_concurrent_effects,
                "default_timeout_ms": self.default_timeout_ms,
    ...
    async def _cleanup_node_resources(self) -> None:
        ...
        emit_log_event(
            LogLevel.INFO,
            "NodeEffectLegacy resources cleaned up",
            {"node_id": str(self.node_id)},
        )
