...
def _bootstrap_user_config(onex_home: Path, force: bool) -> None:
    """Write ~/.onex/config.yaml, normalizing an existing file onto the schema.

    Delegates to ``cli_user_config`` so this command and ``onex config init``
    resolve one schema and one template for the same file (OMN-16037).
    """
    ...
    if config_file.exists():
        ...
        try:
            ...
            sys.exit(1)
        _ensure_state_dirs(onex_home)
        if migrated:
            click.echo(
                f"ONEX user config migrated to the current schema at {config_file}"
            )
        else:
            click.echo(f"ONEX user config already current at {config_file}")
        _echo_next_steps(config_file)
        return
...
def _ensure_state_dirs(onex_home: Path) -> None:
...
def _echo_next_steps(config_file: Path) -> None:
    click.echo("\nNext steps:")
    click.echo(f"  Edit {config_file}")
    click.echo("  Set credentials.LINEAR_API_KEY from https://linear.app/settings/api")
    # `onex health` takes no --mode option; the hint must stay invocable.
    click.echo("  Run: onex health")
