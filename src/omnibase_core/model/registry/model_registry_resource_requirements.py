...
class ModelRegistryResourceRequirements(BaseModel):
    ...
    cpu_score: int = Field(
        ...,
        description="CPU requirement score (1-10 scale)",
        ge=1,
        le=10,
    )
    ...
    memory_score: int = Field(
        ...,
        description="Memory requirement score (1-10 scale)",
        ge=1,
        le=10,
    )
    ...
    disk_space_score: int = Field(
        1,
        description="Disk space requirement score (1-10 scale)",
        ge=1,
        le=10,
    )
    ...
    min_memory_mb: int | None = Field(
        None,
        description="Minimum memory in megabytes",
        ge=1,
    )

    recommended_memory_mb: int | None = Field(
        None,
        description="Recommended memory in megabytes",
        ge=1,
    )

    min_cpu_cores: float | None = Field(None, description="Minimum CPU cores", gt=0)

    recommended_cpu_cores: float | None = Field(
        None,
        description="Recommended CPU cores",
        gt=0,
    )

    disk_iops_required: int | None = Field(
        None,
        description="Disk IOPS required",
        ge=0,
    )

    network_bandwidth_mbps: float | None = Field(
        None,
        description="Network bandwidth required in Mbps",
        ge=0,
    )
