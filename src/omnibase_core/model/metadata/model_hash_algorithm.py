...
class ModelHashAlgorithm(BaseModel):
    ...
    output_format: str = Field(
        "hex", description="Output format for hash", pattern="^(hex|base64|raw)$"
    )

    key: Optional[bytes] = Field(
        None, description="Optional key for keyed hash algorithms (BLAKE2)"
    )

    digest_size: Optional[int] = Field(
        None, description="Digest size for variable-length algorithms"
    )

    @validator("key")
    def validate_key_for_algorithm(cls, v, values):
        ...
        if v is not None:
            if algorithm not in [EnumHashAlgorithm.BLAKE2B, EnumHashAlgorithm.BLAKE2S]:
                raise ValueError(
                    f"Algorithm {algorithm} does not support keyed hashing"
                )
        return v

    @validator("digest_size")
    def validate_digest_size(cls, v, values):
        """Validate digest size for algorithms that support it."""
        if v is not None:
            algorithm = values.get("algorithm")
            if algorithm == EnumHashAlgorithm.BLAKE2B and (v < 1 or v > 64):
                raise ValueError("BLAKE2B digest size must be between 1 and 64")
            elif algorithm == EnumHashAlgorithm.BLAKE2S and (v < 1 or v > 32):
                raise ValueError("BLAKE2S digest size must be between 1 and 32")
            elif algorithm not in [
                EnumHashAlgorithm.BLAKE2B,
                EnumHashAlgorithm.BLAKE2S,
            ]:
                raise ValueError(
                    f"Algorithm {algorithm} does not support variable digest size"
                )
        return v
