...
class ModelSuccessCriteria(BaseModel):
    ...
    secondary_metrics: List[str] = Field(
        default_factory=list, description="Secondary success metrics"
    )
    secondary_thresholds: List[float] = Field(
        default_factory=list, description="Secondary metric thresholds"
    )
    ...
    failure_conditions: List[str] = Field(
        default_factory=list, description="Conditions that indicate failure"
    )
    timeout_seconds: Optional[int] = Field(
        default=None, description="Timeout for success evaluation"
    )
    evaluation_mode: str = Field(
        default="all_pass", description="How to evaluate multiple criteria"
    )
