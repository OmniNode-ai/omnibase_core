...
class ModelAgentResourceMetrics(BaseModel):
    ...
    cpu_usage_percent: float = Field(
        description="CPU usage percentage for this agent",
        ge=0,
        le=100,
    )
    memory_usage_mb: float = Field(
        description="Memory usage in MB for this agent",
        ge=0,
    )
    ...
    heap_size_mb: float | None = Field(
        None,
        description="Heap size in MB for managed languages",
        ge=0,
    )
...
class ModelAgentPerformanceMetrics(BaseModel):
    ...
    min_task_duration_ms: float = Field(
        description="Minimum task completion time in milliseconds",
        ge=0,
    )
    max_task_duration_ms: float = Field(
        description="Maximum task completion time in milliseconds",
        ge=0,
    )
...
class ModelAgentHealthIndicators(BaseModel):
    ...
    last_error_timestamp: datetime | None = Field(
        None,
        description="Timestamp of last error encountered",
    )
    last_error_message: str | None = Field(
        None,
        description="Message from last error encountered",
    )
...
class ModelAgentMonitoringMetrics(BaseModel):
    ...
    node_id: str | None = Field(None, description="ID of the node hosting the agent")
    ...
    collection_interval_ms: int = Field(
        description="Interval between metric collections in milliseconds",
        ge=1000,
    )
    ...
    configuration_hash: str | None = Field(
        None,
        description="Hash of current agent configuration",
    )
    configuration_version: str | None = Field(
        None,
        description="Version of current agent configuration",
    )
    ...
    correlation_id: UUID | None = Field(
        None,
        description="Correlation ID for related monitoring events",
    )
    session_id: str | None = Field(
        None,
        description="Session ID if agent is part of a user session",
    )
...
class ModelAgentMonitoringSummary(BaseModel):
    ...
    total_cpu_usage_percent: float = Field(
        description="Total CPU usage across all agents",
        ge=0,
    )
    total_memory_usage_mb: float = Field(
        description="Total memory usage across all agents",
        ge=0,
    )
    ...
    average_memory_usage_mb: float = Field(
        description="Average memory usage per agent",
        ge=0,
    )

    # Aggregate performance metrics
    total_tasks_completed: int = Field(
        description="Total tasks completed across all agents",
        ge=0,
    )
    total_tasks_failed: int = Field(
        description="Total tasks failed across all agents",
        ge=0,
    )
    overall_success_rate_percent: float = Field(
        description="Overall success rate across all agents",
        ge=0,
        le=100,
    )
    ...
    agents_with_high_resource_usage: int = Field(
        description="Number of agents with high resource usage",
        ge=0,
    )
    agents_with_errors: int = Field(
        description="Number of agents that have had recent errors",
        ge=0,
    )
    agents_needing_attention: int = Field(
        description="Number of agents requiring attention",
        ge=0,
    )
    ...
    active_alerts: int = Field(
        description="Number of active alerts related to agents",
        ge=0,
    )
    scaling_recommendations: list[str] = Field(
        default_factory=list,
        description="Recommendations for scaling agents",
    )
...
class ModelAgentScalingTrigger(BaseModel):
    ...
    duration_seconds: int = Field(
        description="Duration threshold must be exceeded for trigger",
        ge=1,
    )
    cooldown_seconds: int = Field(
        description="Cooldown period before next scaling action",
        ge=1,
    )
    scaling_action: str = Field(
        description="Action to take (scale_up, scale_down, restart)",
    )
    target_agent_count: int | None = Field(
        None,
        description="Target number of agents after scaling",
        ge=0,
    )
