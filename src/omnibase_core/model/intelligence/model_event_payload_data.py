...
class ModelEventPayloadData(BaseModel):
    ...
    pattern_id: str | None = Field(
        default=None,
        description="Pattern identifier if applicable",
    )
    rule_id: str | None = Field(
        default=None,
        description="Rule identifier if applicable",
    )
    ...
    confidence_score: float | None = Field(
        default=None,
        description="Confidence score for event",
    )
    ...
    # Boolean flags
    is_validated: bool | None = Field(
        default=None,
        description="Whether event is validated",
    )
    requires_promotion: bool | None = Field(
        default=None,
        description="Whether promotion is required",
    )
    is_shadow_mode: bool | None = Field(
        default=None,
        description="Whether in shadow mode",
    )

    # Counts and metrics
    event_count: int | None = Field(default=None, description="Number of events")
...
class ModelEventMetadata(BaseModel):
    ...
    # Source information
    source_type: str | None = Field(default=None, description="Type of event source")
    source_version: str | None = Field(
        default=None,
        description="Source component version",
    )
    source_environment: str | None = Field(
        default=None,
    ...
    processing_node: str | None = Field(
        default=None,
        description="Node that processed event",
    )
    queue_name: str | None = Field(
        default=None,
        description="Queue that handled event",
    )

    # Timing information
    enqueued_at: datetime | None = Field(
        default=None,
        description="When event was queued",
    )
    ...
    session_context: str | None = Field(default=None, description="Session context")
...
class ModelValidationResult(BaseModel):
    ...
    recall_score: float | None = Field(default=None, description="Validation recall")

    # Validation details
    rules_tested: list[str] = Field(
        default_factory=list,
        description="Rules that were tested",
    )
    ...
    validation_completed_at: datetime | None = Field(
        default=None,
...
class ModelWorkflowContextData(BaseModel):
    ...
    parent_workflow_id: str | None = Field(
        default=None,
        description="Parent workflow if nested",
    )
    ...
    failure_handling: str | None = Field(
        default=None,
        description="Failure handling strategy",
    )

    # Progress tracking
    total_steps: int | None = Field(default=None, description="Total workflow steps")
    ...
    # Resource usage
    cpu_usage_seconds: float | None = Field(
        default=None,
    ...
    dependent_workflows: list[str] = Field(
        default_factory=list,
        description="Dependent workflows",
    )
...
class ModelOperationContext(BaseModel):
    ...
    initiated_by: str | None = Field(
        default=None,
        description="Who/what initiated operation",
    )
...
class ModelSuccessCriteria(BaseModel):
    ...
    min_success_percentage: float | None = Field(
        default=None,
        description="Minimum success percentage",
    )
    required_completion_percentage: float | None = Field(
        default=None,
        description="Required completion percentage",
    )

    # Boolean criteria
    all_must_succeed: bool = Field(
        default=False,
        description="Whether all sub-operations must succeed",
    )
    ...
    timeout_behavior: str = Field(
        default="fail",
        description="Behavior on timeout (fail, retry, partial)",
    )
...
class ModelBatchProcessingMetadata(BaseModel):
    ...
    max_concurrent_items: int | None = Field(
        default=None,
        description="Max concurrent processing",
    )
    chunk_size: int | None = Field(default=None, description="Processing chunk size")

    # Performance requirements
    target_throughput_per_second: float | None = Field(
        default=None,
    ...
    failure_threshold_percentage: float = Field(
        default=0.1,
        description="Failure threshold before aborting",
    )
...
class ModelComponentContext(BaseModel):
    ...
    performance_characteristics: dict[str, str | int | float] = Field(
        default_factory=dict,
        description="Performance characteristics",
    )

    # State information
    current_load: float | None = Field(
        default=None,
        description="Current load percentage",
    )
    health_status: str = Field(default="healthy", description="Component health status")
    availability_percentage: float | None = Field(
        default=None,
...
class ModelErrorContext(BaseModel):
    ...
    event_id: str | None = Field(
        default=None,
        description="Related event ID if applicable",
    )
    ...
    execution_duration_ms: float | None = Field(
        default=None,
        description="Time before error",
    )

    # System state
    system_state: dict[str, str | int | float | bool] = Field(
        default_factory=dict,
        description="Relevant system state at error time",
    )
...
class ModelDebugContext(BaseModel):
    ...
    trace_level: str = Field(
        default="info",
        description="Trace detail level (debug, info, warn, error)",
    )

    # Execution context
    execution_path: list[str] = Field(
        default_factory=list,
        description="Execution path through components",
    )
    method_calls: list[dict[str, str]] = Field(
        default_factory=list,
        description="Method call chain",
    )

    # Performance data
    performance_metrics: dict[str, float] = Field(
        default_factory=dict,
        description="Performance metrics during execution",
    )
    ...
    state_snapshots: dict[str, dict] = Field(
        default_factory=dict,
        description="Component state snapshots at key points",
    )
    ...
    environment_info: dict[str, str] = Field(
        default_factory=dict,
        description="Environment and configuration info",
    )
