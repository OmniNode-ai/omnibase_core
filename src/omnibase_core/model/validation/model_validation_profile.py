...
class ModelValidationProfile(BaseModel):
    ...
    profile_name: str = Field(
        ..., description="Name of the validation profile", min_length=1
    )
    max_failures_allowed: int = Field(
        default=0, description="Maximum number of test failures allowed", ge=0
    )
    timeout_seconds: int = Field(
        default=600, description="Maximum validation time", gt=0, le=7200
    )
    mode: EnumValidationMode = Field(
        default=EnumValidationMode.STRICT, description="Validation mode"
    )
