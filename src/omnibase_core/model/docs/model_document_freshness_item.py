...
class ModelDocumentFreshnessItem(BaseModel):
    ...
    freshness_score: float = Field(
        ge=0.0,
        le=1.0,
        description="Freshness score (0=stale, 1=fresh)",
    )
    last_modified: datetime = Field(description="Last modification timestamp")
    staleness_days: int = Field(
        ge=0,
        description="Number of days since last modification",
    )
    risk_level: EnumDocumentFreshnessRiskLevel = Field(
        description="Risk level based on staleness",
    )
    git_last_commit: datetime | None = Field(
        default=None,
        description="Last Git commit timestamp for this file",
    )
    dependency_count: int = Field(
        default=0,
        ge=0,
        description="Number of dependencies identified",
    )
