...
class ModelFreshnessSummary(BaseModel):
    ...
    critical_count: int = Field(
        ge=0, description="Number of critically stale documents"
    )
    avg_freshness_score: float = Field(
        ge=0.0, le=1.0, description="Average freshness score across all documents"
    )
    oldest_document: Optional[str] = Field(
        default=None, description="Path to the oldest document"
    )
    newest_document: Optional[str] = Field(
        default=None, description="Path to the newest document"
    )
