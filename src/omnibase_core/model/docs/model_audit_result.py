...
from omnibase_core.model.docs.model_document_lifecycle import \
...
class ModelAuditFinding(ModelOnexInputState):
    ...
    line_number: Optional[int] = Field(
        None, description="Line number where finding was detected"
    )
    section: Optional[str] = Field(
        None, description="Document section where finding was detected"
    )
    context: Optional[str] = Field(
        None, description="Surrounding context for the finding"
    )
    ...
    estimated_effort_hours: Optional[float] = Field(
        None, description="Estimated effort to fix"
    )
    auto_fixable: bool = Field(
        default=False, description="Whether this can be automatically fixed"
    )

    # AI analysis (if applicable)
    ai_confidence: Optional[float] = Field(
        None, description="AI confidence in this finding (0.0-1.0)"
    )
    ai_reasoning: Optional[str] = Field(
        None, description="AI reasoning for this finding"
    )

    # Tracking
    detected_at: datetime = Field(
        default_factory=datetime.utcnow, description="When finding was detected"
    )
    resolved: bool = Field(
        default=False, description="Whether finding has been resolved"
    )
    resolved_at: Optional[datetime] = Field(
        None, description="When finding was resolved"
    )
...
class ModelQualityMetrics(ModelOnexInputState):
    ...
    code_examples_score: float = Field(
        default=1.0, description="Quality of code examples (0.0-1.0)"
    )
    technical_accuracy_score: float = Field(
        default=1.0, description="Technical accuracy (0.0-1.0)"
    )
    ...
    quality_trend: str = Field(
        default="stable", description="Quality trend (improving, stable, declining)"
    )

    # Benchmarking
    peer_comparison_score: Optional[float] = Field(
        None, description="Score compared to similar documents"
    )
    industry_standard_score: Optional[float] = Field(
        None, description="Score against industry standards"
    )
...
class ModelComplianceMetrics(ModelOnexInputState):
    ...
    contract_yaml_compliance: float = Field(
        default=1.0, description="Contract YAML compliance for nodes (0.0-1.0)"
    )
    architecture_compliance: float = Field(
        default=1.0, description="Architecture documentation compliance (0.0-1.0)"
    )

    # Accessibility compliance
    accessibility_score: float = Field(
        default=1.0, description="Accessibility compliance (0.0-1.0)"
    )
    alt_text_coverage: float = Field(
        default=1.0, description="Alt text coverage for images (0.0-1.0)"
    )

    # Security compliance
    sensitive_info_score: float = Field(
        default=1.0, description="No sensitive information exposed (0.0-1.0)"
    )
    credential_exposure_score: float = Field(
        default=1.0, description="No credentials exposed (0.0-1.0)"
    )
    ...
    compliance_issues_count: int = Field(
        default=0, description="Number of compliance issues found"
    )
    critical_compliance_issues: int = Field(
        default=0, description="Number of critical compliance issues"
    )
    ...
    def validate_compliance_scores(cls, v):
        if not 0.0 <= v <= 1.0:
...
class ModelDocumentAuditResult(ModelOnexInputState):
    ...
    audit_completed_at: Optional[datetime] = Field(
        None, description="When audit completed"
    )
    audit_duration_ms: int = Field(
        default=0, description="Audit duration in milliseconds"
    )
    ...
    document_line_count: int = Field(
        default=0, description="Number of lines in document"
    )

    # Audit results
    findings: List[ModelAuditFinding] = Field(
        default_factory=list, description="All findings from the audit"
    )
    quality_metrics: Optional[ModelQualityMetrics] = Field(
        None, description="Quality assessment metrics"
    )
    compliance_metrics: Optional[ModelComplianceMetrics] = Field(
        None, description="Compliance assessment metrics"
    )
    ...
    medium_findings: int = Field(
        default=0, description="Number of medium severity findings"
    )
    low_findings: int = Field(default=0, description="Number of low severity findings")
    info_findings: int = Field(
        default=0, description="Number of informational findings"
    )

    # Overall assessment
    overall_score: float = Field(..., description="Overall audit score (0.0-1.0)")
    pass_threshold: float = Field(
        default=0.7, description="Minimum score to pass audit"
    )
    audit_passed: bool = Field(..., description="Whether document passed the audit")

    # Recommendations
    priority_actions: List[str] = Field(
        default_factory=list, description="High-priority recommended actions"
    )
    estimated_fix_effort_hours: float = Field(
        default=0.0, description="Estimated total effort to address findings"
    )
    next_audit_recommended_date: Optional[datetime] = Field(
        None, description="When next audit is recommended"
    )

    # AI analysis (if applicable)
    ai_analysis_performed: bool = Field(
        default=False, description="Whether AI analysis was performed"
    )
    ai_overall_assessment: Optional[str] = Field(
        None, description="AI's overall assessment of the document"
    )
    ai_improvement_suggestions: List[str] = Field(
        default_factory=list, description="AI-generated improvement suggestions"
    )
...
class ModelBatchAuditResult(ModelOnexInputState):
    ...
    batch_completed_at: Optional[datetime] = Field(
        None, description="When batch audit completed"
    )
    total_duration_ms: int = Field(
        default=0, description="Total duration in milliseconds"
    )
    ...
    documents_successfully_audited: int = Field(
        default=0, description="Documents successfully audited"
    )
    documents_failed_audit: int = Field(
        default=0, description="Documents that failed to audit"
    )
    documents_skipped: int = Field(default=0, description="Documents skipped")

    # Individual results
    document_results: List[ModelDocumentAuditResult] = Field(
        default_factory=list, description="Results for each document"
    )

    # Aggregate statistics
    total_findings_across_all_docs: int = Field(
        default=0, description="Total findings across all documents"
    )
    average_quality_score: float = Field(
        default=0.0, description="Average quality score across documents"
    )
    average_compliance_score: float = Field(
        default=0.0, description="Average compliance score"
    )
    documents_passed_audit: int = Field(
        default=0, description="Number of documents that passed audit"
    )

    # Distribution of findings by severity
    critical_findings_total: int = Field(
        default=0, description="Total critical findings"
    )
    high_findings_total: int = Field(
        default=0, description="Total high severity findings"
    )
    medium_findings_total: int = Field(
        default=0, description="Total medium severity findings"
    )
    low_findings_total: int = Field(
        default=0, description="Total low severity findings"
    )

    # Top issues
    most_common_findings: List[Dict] = Field(
        default_factory=list, description="Most frequently occurring findings"
    )
    documents_requiring_urgent_attention: List[str] = Field(
        default_factory=list, description="Documents needing urgent fixes"
    )

    # Recommendations and insights
    overall_recommendations: List[str] = Field(
        default_factory=list, description="Overall recommendations for the document set"
    )
    ...
    estimated_total_fix_effort_hours: float = Field(
        default=0.0, description="Total estimated effort to address all findings"
    )

    # Trend analysis
    quality_trend_analysis: Optional[Dict] = Field(
        None, description="Quality trend analysis if historical data available"
    )
    compliance_trend_analysis: Optional[Dict] = Field(
        None, description="Compliance trend analysis"
    )

    # Performance metrics
    audit_performance_metrics: Dict = Field(
        default_factory=dict, description="Performance metrics for the audit process"
    )

    @validator("average_quality_score", "average_compliance_score")
    def validate_average_scores(cls, v):
        if not 0.0 <= v <= 1.0:
            raise ValueError("Average score must be between 0.0 and 1.0")
        return v
...
class ModelAuditConfiguration(ModelOnexInputState):
    ...
    document_type_filters: List[EnumDocumentType] = Field(
        default_factory=list, description="Document types to include (empty = all)"
    )
    directory_patterns: List[str] = Field(
        default_factory=list, description="Directory patterns to include"
    )
    excluded_patterns: List[str] = Field(
        default_factory=list, description="Patterns to exclude from audit"
    )

    # Quality thresholds
    minimum_quality_score: float = Field(
        default=0.7, description="Minimum acceptable quality score"
    )
    minimum_compliance_score: float = Field(
        default=0.8, description="Minimum acceptable compliance score"
    )

    # AI analysis settings
    enable_ai_analysis: bool = Field(
        default=True, description="Whether to enable AI-powered analysis"
    )
    ai_analysis_model: str = Field(
        default="phi3:latest", description="AI model to use for analysis"
    )
    ai_batch_size: int = Field(default=5, description="Batch size for AI analysis")
    ai_timeout_seconds: int = Field(
        default=120, description="Timeout for AI analysis per document"
    )

    # Performance settings
    max_concurrent_audits: int = Field(
        default=4, description="Maximum concurrent audit processes"
    )
    audit_timeout_seconds: int = Field(
        default=300, description="Timeout for individual document audits"
    )
    enable_caching: bool = Field(
        default=True, description="Whether to cache audit results"
    )
    cache_ttl_hours: int = Field(default=24, description="Cache time-to-live in hours")

    # Reporting settings
    generate_detailed_reports: bool = Field(
        default=True, description="Whether to generate detailed reports"
    )
    include_ai_reasoning: bool = Field(
        default=True, description="Include AI reasoning in reports"
    )
    ...
    notify_on_critical_findings: bool = Field(
        default=True, description="Send notifications for critical findings"
    )
    notification_recipients: List[str] = Field(
        default_factory=list, description="Recipients for notifications"
    )

    @validator("minimum_quality_score", "minimum_compliance_score")
    def validate_minimum_scores(cls, v):
        if not 0.0 <= v <= 1.0:
