...
class ModelUserMessage(BaseModel):
    ...
    timestamp: datetime = Field(
        default_factory=datetime.utcnow, description="When the user message was sent"
    )
    content: str = Field(description="The user's message content")
    message_type: EnumMessageType = Field(
        default=EnumMessageType.TEXT, description="Type of message"
    )
    ...
    # Context information
    model_requested: Optional[str] = Field(
        None, description="AI model that was requested"
    )
    system_prompt: Optional[str] = Field(
        None, description="System prompt sent with the request"
    )
    max_tokens: Optional[int] = Field(None, description="Maximum tokens requested")
    temperature: Optional[float] = Field(
        None, description="Temperature setting for the request"
    )

    # Full request data
    full_request_body: Dict[str, Any] = Field(
        default_factory=dict, description="Complete request body for analysis"
    )
...
class ModelClaudeResponse(BaseModel):
    ...
    text_content: Optional[str] = Field(
        None, description="Claude's text response before tool calls"
    )

    # Tool request information
    has_tool_requests: bool = Field(
        default=False, description="Whether this response contains tool requests"
    )
    tool_requests: List[Dict[str, Any]] = Field(
        default_factory=list, description="List of tool requests made by Claude"
    )

    # Response metadata
    model_used: Optional[str] = Field(
        None, description="AI model that generated this response"
    )
    stop_reason: Optional[str] = Field(None, description="Why the response stopped")
    tokens_used: Optional[int] = Field(
        None, description="Number of tokens used in the response"
    )

    # Full response data
    full_response_data: Dict[str, Any] = Field(
        default_factory=dict, description="Complete response data for analysis"
    )
...
class ModelConversationTurn(BaseModel):
    ...
    # Turn analysis
    turn_successful: Optional[bool] = Field(
        None, description="Whether this turn was successful (if determinable)"
    )
    user_satisfaction: Optional[str] = Field(
        None, description="Indicators of user satisfaction with response"
    )
    ...
    completed_at: Optional[datetime] = Field(
        None, description="When this conversation turn completed"
    )
...
class ModelConversationSession(BaseModel):
    ...
    last_activity_at: datetime = Field(
        default_factory=datetime.utcnow, description="When the last activity occurred"
    )
    ended_at: Optional[datetime] = Field(
        None, description="When the conversation session ended"
    )
    ...
    total_tokens_used: int = Field(
        default=0, description="Total tokens used in this session"
    )
    total_tool_requests: int = Field(
        default=0, description="Total tool requests made in this session"
    )

    # Session context
    session_type: str = Field(
        default="claude_code", description="Type of conversation session"
    )
    user_agent: Optional[str] = Field(None, description="User agent from requests")

    # OmniMemory integration
    omnimemory_rules_applied: int = Field(
        default=0, description="Number of OmniMemory rules applied in this session"
    )
    context_injection_count: int = Field(
        default=0, description="Number of times context was injected"
    )
...
class ModelConversationEmbedding(BaseModel):
    ...
    embedding_model: str = Field(
        default="mxbai-embed-large", description="Model used to generate the embedding"
    )
    ...
    # Embedding metadata
    created_at: datetime = Field(
        default_factory=datetime.utcnow, description="When this embedding was created"
    )
    embedding_time_ms: Optional[float] = Field(
        None, description="Time taken to generate embedding in milliseconds"
    )
