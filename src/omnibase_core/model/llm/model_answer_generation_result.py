...
class ModelAnswerCandidate(BaseModel):
    ...
    quality_score: float = Field(
        ..., ge=0.0, le=1.0, description="Answer quality score"
    )
    source_coverage: float = Field(
        ..., ge=0.0, le=1.0, description="Percentage of sources utilized in answer"
    )
    model_used: str = Field(..., description="Model used for generation")
    generation_time_ms: int = Field(
        ..., ge=0, description="Generation time in milliseconds"
    )
    metadata: dict = Field(
...
class ModelAnswerGenerationResult(ModelOnexInputState):
    ...
    context_documents: List[ModelRetrievedDocument] = Field(
        default_factory=list, description="Documents used as context for generation"
    )
    ...
    sources_cited: List[str] = Field(
        default_factory=list, description="Sources cited in the final answer"
    )
    overall_confidence: float = Field(
        ..., ge=0.0, le=1.0, description="Overall confidence in the generated answer"
    )
    processing_time_ms: int = Field(
        ..., ge=0, description="Total processing time in milliseconds"
    )
    tokens_used: Optional[int] = Field(
        default=None, ge=0, description="Total tokens used for generation"
    )
    fallback_used: bool = Field(
        default=False, description="Whether fallback generation was used"
    )
