...
from omnibase_core.model.llm.model_gemini_types import \
    ModelGeminiGenerationConfig
from omnibase_core.model.llm.model_provider_metadata import \
...
class ModelProviderConfig(BaseModel):
    ...
    retry_delay_seconds: int = Field(
        default=1, ge=0, le=60, description="Delay between retry attempts"
    )

    health_check_interval_seconds: int = Field(
        default=60, ge=10, le=3600, description="Interval between health checks"
    )
    ...
    additional_config: Optional[ModelProviderMetadata] = Field(
        default=None, description="Provider-specific additional configuration"
    )
...
class ModelOllamaConfig(ModelProviderConfig):
    ...
    verify_ssl: bool = Field(
        default=True, description="Verify SSL certificates for HTTPS"
    )
    ...
    default_model: str = Field(
        default="mistral:latest", description="Default model to use if none specified"
    )

    auto_pull_models: bool = Field(
        default=True, description="Automatically pull models if not available"
    )
...
class ModelOpenAIConfig(ModelProviderConfig):
    ...
    default_model: str = Field(
        default="gpt-3.5-turbo", description="Default OpenAI model to use"
    )

    max_cost_per_request: Optional[float] = Field(
        default=None, ge=0.0, description="Maximum cost per request in USD"
    )

    daily_cost_limit: Optional[float] = Field(
        default=None, ge=0.0, description="Daily cost limit in USD"
    )
...
class ModelAnthropicConfig(ModelProviderConfig):
    ...
    default_model: str = Field(
        default="claude-3-sonnet-20240229", description="Default Anthropic model to use"
    )
...
class ModelGeminiConfig(ModelProviderConfig):
    ...
    default_model: str = Field(
        default="gemini-1.5-flash", description="Default model to use if none specified"
    )
    ...
    generation_config: Optional[ModelGeminiGenerationConfig] = Field(
        default=None, description="Default generation configuration"
    )
