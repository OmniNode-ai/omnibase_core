...
class ModelPerformanceStatistics(BaseModel):
    ...
    collection_period_seconds: int = Field(
        gt=0,
        description="Statistics collection period in seconds",
    )
...
class ModelPerformanceMetadata(BaseModel):
    ...
    operation_type: str = Field(
        min_length=1,
        description="Type of operation being monitored",
    )
    ...
    cache_hit: bool | None = Field(
        default=None,
        description="Whether operation resulted in cache hit",
    )
    error_occurred: bool | None = Field(
        default=None,
        description="Whether an error occurred during operation",
    )
