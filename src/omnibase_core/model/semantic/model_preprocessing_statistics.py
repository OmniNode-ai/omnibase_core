...
class ModelPreprocessingStatistics(BaseModel):
    ...
    average_chunk_size: float | None = Field(
        default=None,
    ...
    average_processing_time_per_doc_ms: float | None = Field(
        default=None,
    ...
    memory_usage_mb: float | None = Field(
        default=None,
        description="Peak memory usage during processing in MB",
    )

    documents_with_warnings: int = Field(
        default=0,
        description="Number of documents that had warnings",
    )

    documents_skipped: int = Field(
        default=0,
        description="Number of documents skipped due to filters",
    )
