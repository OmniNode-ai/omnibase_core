...
from omnibase_core.model.semantic.model_retrieval_filters import ModelRetrievalFilters
...
class ModelCacheInputState(ModelOnexInputState):
    ...
    cache_key: str | None = Field(
        default=None,
        description="Cache key for retrieval operations",
    )

    query_text: str | None = Field(
        default=None,
        description="Original query text for cache key generation",
    )

    filters: ModelRetrievalFilters | None = Field(
        default=None,
        description="Search filters for cache key generation",
    )

    retriever_type: str | None = Field(
        default=None,
        description="Type of retriever used for search",
    )

    results_to_cache: list[ModelCacheResult] | None = Field(
        default=None,
        description="Results to store in cache",
    )

    embedding_text: str | None = Field(
        default=None,
        description="Text content for embedding caching",
    )

    embedding_vector: list[float] | None = Field(
        default=None,
        description="Embedding vector to cache",
    )
    ...
    ttl_override: int | None = Field(
        default=None,
        ge=1,
        description="Override TTL for this cache entry in seconds",
    )
