...
from omnibase_core.model.semantic.model_document_metadata import \
...
class ModelPreprocessingOutputState(ModelOnexOutputState):
    ...
    processed_documents: List[ModelDocumentMetadata] = Field(
        description="List of processed documents with chunks", default_factory=list
    )
    ...
    quality_metrics: Dict[str, float] = Field(
        description="Quality metrics for processed documents", default_factory=dict
    )

    warnings: List[str] = Field(
        description="Non-fatal warnings from processing", default_factory=list
    )
    ...
    documents_processed: int = Field(
        default=0, description="Number of documents successfully processed"
    )
