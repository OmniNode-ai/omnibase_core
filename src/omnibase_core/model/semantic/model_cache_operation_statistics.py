...
class ModelCacheOperationStatistics(BaseModel):
    ...
    postgres_cache_hits: int = Field(
        default=0,
        description="Hits from PostgreSQL cache",
    )

    average_lookup_time_ms: float | None = Field(
        default=None,
        description="Average cache lookup time in milliseconds",
    )

    average_storage_time_ms: float | None = Field(
        default=None,
        description="Average cache storage time in milliseconds",
    )

    total_entries_stored: int = Field(
        default=0,
        description="Total number of entries stored in session",
    )

    total_entries_evicted: int = Field(
        default=0,
        description="Total number of entries evicted",
    )
    ...
    disk_usage_bytes: int | None = Field(
        default=None,
        description="Current disk usage by cache in bytes",
    )
    ...
    expired_entries_cleaned: int = Field(
        default=0,
        description="Number of expired entries cleaned up",
    )

    duplicate_requests_deduplicated: int = Field(
        default=0,
        description="Number of duplicate requests served from cache",
    )
