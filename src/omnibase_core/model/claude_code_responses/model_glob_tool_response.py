...
class ModelGlobToolResponse(BaseModel):
    ...
    pattern_type: Literal["glob", "regex", "literal"] = Field(
        default="glob",
        description="Type of pattern matching used",
    )
    ...
    hidden_files_included: bool = Field(
        default=False,
        description="Whether hidden files were included",
    )
    sorted_results: bool = Field(
        default=True,
        description="Whether results were sorted",
    )
    execution_time_ms: int | None = Field(
        None,
        description="Pattern matching execution time",
        ge=0,
    )
