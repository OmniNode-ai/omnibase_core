...
class ModelWebSearchResult(BaseModel):
    ...
    display_url: Optional[str] = Field(None, description="Display URL shown in results")
    relevance_score: Optional[float] = Field(
        None, description="Relevance score if available", ge=0, le=1
    )
    published_date: Optional[datetime] = Field(
        None, description="Publication date if available"
    )
    source_domain: Optional[str] = Field(
        None, description="Domain of the result source"
    )
...
class ModelWebSearchToolResponse(BaseModel):
    ...
    search_region: Optional[str] = Field(
        None, description="Geographic region for search"
    )
    safe_search: bool = Field(
        default=True, description="Whether safe search was enabled"
    )
