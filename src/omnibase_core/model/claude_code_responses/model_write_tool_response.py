...
class ModelWriteToolResponse(BaseModel):
    ...
    backup_created: bool = Field(
        default=False, description="Whether backup was created before overwrite"
    )
    backup_path: Optional[str] = Field(
        None, description="Path to backup file if created"
    )
    permissions_set: Optional[str] = Field(
        None, description="File permissions set after write"
    )
