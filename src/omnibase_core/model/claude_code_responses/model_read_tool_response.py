...
class ModelFileMetadata(BaseModel):
    ...
    is_symlink: bool = Field(
        default=False,
        description="Whether file is a symbolic link",
    )
    created: datetime | None = Field(None, description="Creation timestamp")
    owner: str | None = Field(None, description="File owner")
    group: str | None = Field(None, description="File group")
    symlink_target: str | None = Field(
        None,
        description="Target of symbolic link if applicable",
    )
...
class ModelReadToolResponse(BaseModel):
    ...
    is_binary: bool = Field(
        default=False,
        description="Whether file is detected as binary",
    )
    line_offset: int | None = Field(
        None,
        description="Starting line number if offset was used",
        ge=1,
    )
    line_limit: int | None = Field(
        None,
        description="Maximum lines read if limit was applied",
        ge=1,
    )
    ...
    metadata: ModelFileMetadata | None = Field(
        None,
