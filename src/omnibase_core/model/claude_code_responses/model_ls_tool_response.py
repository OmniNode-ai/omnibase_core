...
class ModelDirectoryEntry(BaseModel):
    ...
    size: int | None = Field(None, description="Size in bytes for files", ge=0)
    is_executable: bool = Field(
        default=False,
        description="Whether entry is executable",
    )
    symlink_target: str | None = Field(
        None,
        description="Target if entry is symlink",
    )
...
class ModelLSToolResponse(BaseModel):
    ...
    hidden_entries_count: int = Field(
        default=0,
        description="Number of hidden entries found",
        ge=0,
    )
    symlinks_count: int = Field(
        default=0,
        description="Number of symbolic links found",
        ge=0,
    )
    ignore_patterns: list[str] | None = Field(
        None,
        description="Ignore patterns that were applied",
    )
    recursive: bool = Field(default=False, description="Whether listing was recursive")
    execution_time_ms: int | None = Field(
        None,
        description="Directory listing execution time",
        ge=0,
    )
