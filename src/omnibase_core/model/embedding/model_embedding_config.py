...
from omnibase_core.model.embedding.model_embedding_provider_config import \
...
class ModelEmbeddingConfig(BaseModel):
    ...
    fallback_providers: List[ModelEmbeddingProviderConfig] = Field(
        description="Ordered list of fallback providers", default_factory=list
    )

    cloud_fallback_provider: Literal["gemini", "openai", "disabled"] = Field(
        description="Cloud provider to use as final fallback", default="gemini"
    )

    max_retry_attempts: int = Field(
        description="Maximum number of retry attempts per provider", default=3, ge=1
    )

    connection_timeout_seconds: int = Field(
        description="Connection timeout for embedding requests", default=30, ge=1
    )
