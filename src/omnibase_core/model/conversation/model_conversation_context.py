...
class ModelConversationInteraction(BaseModel):
    ...
    tools_used: List[str] = Field(
        default_factory=list, description="List of tools used in this interaction"
    )
    outcome: str = Field(
        default="in_progress",
    ...
    task_context: Optional[str] = Field(
        default=None, description="Optional context about the task type"
    )
...
class ModelConversationSessionSummary(BaseModel):
    ...
    tools_used: List[str] = Field(
        default_factory=list, description="All tools used in the session"
    )
...
class ModelConversationCaptureResult(BaseModel):
    ...
    interaction_id: Optional[str] = Field(
        default=None, description="ID of the captured interaction"
    )
    session_id: Optional[str] = Field(default=None, description="Session ID")
    buffer_size: int = Field(default=0, description="Current buffer size")
    should_flush: bool = Field(
        default=False, description="Whether buffer should be flushed"
    )
...
class ModelConversationFlushResult(BaseModel):
    ...
    entry_id: Optional[str] = Field(
        default=None, description="Debug entry ID if stored"
    )
    flushed_count: int = Field(default=0, description="Number of interactions flushed")
    session_id: Optional[str] = Field(default=None, description="Session ID")
    error: Optional[str] = Field(
        default=None, description="Error message if flush failed"
    )
    message: Optional[str] = Field(
        default=None, description="Additional status message"
    )
...
class ModelConversationQueryResult(BaseModel):
    ...
    results: List[ModelConversationInteraction] = Field(
        default_factory=list, description="List of matching conversation interactions"
    )
    error: Optional[str] = Field(
...
class ModelConversationSessionStatus(BaseModel):
    ...
    total_interactions: int = Field(
        default=0, description="Total interactions in session"
    )
    buffer_size: int = Field(default=0, description="Current buffer size")
    buffer_limit: int = Field(
        default=5, description="Buffer size limit before auto-flush"
    )
    needs_flush: bool = Field(
        default=False, description="Whether buffer needs flushing"
    )
    capture_enabled: bool = Field(
        default=True, description="Whether capture is enabled"
    )
...
class ModelConversationMemoryStats(BaseModel):
    ...
    error: Optional[str] = Field(
        default=None, description="Error message if stats retrieval failed"
    )
...
class ModelToolExecutionHook(BaseModel):
    ...
    tool_result: Optional[str] = Field(
        default=None, description="Result or outcome of the tool"
    )
    execution_time_ms: float = Field(
        default=0.0, description="Tool execution time in milliseconds"
    )
    success: bool = Field(
...
class ModelConversationHookStatus(BaseModel):
    ...
    tools_in_current_interaction: List[str] = Field(
        default_factory=list, description="List of tools used in current interaction"
    )
