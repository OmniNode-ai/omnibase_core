...
from omnibase_core.model.agent.model_agent_health_summary import \
    ModelAgentHealthSummary
from omnibase_core.model.agent.model_device_agent_info import \
    ModelDeviceAgentInfo
...
class ModelAgentSummary(BaseModel):
    ...
    by_device: Dict[str, ModelDeviceAgentInfo] = Field(
        default_factory=dict, description="Agent breakdown by device identifier"
    )
    by_role: Dict[str, ModelRoleAgentInfo] = Field(
        default_factory=dict, description="Agent breakdown by role/capability"
    )
