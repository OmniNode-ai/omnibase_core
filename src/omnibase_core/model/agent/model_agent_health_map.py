...
class ModelAgentHealthMap(BaseModel):
    ...
    health_status: Dict[str, EnumAgentHealth] = Field(
        default_factory=dict, description="Map of agent IDs to health status"
    )

    last_updated: Dict[str, datetime] = Field(
        default_factory=dict, description="Map of agent IDs to last health update time"
    )
    ...
    def get_health(self, agent_id: str) -> Optional[EnumAgentHealth]:
        """Get health status for a specific agent."""
        return self.health_status.get(agent_id)

    def get_last_update(self, agent_id: str) -> Optional[datetime]:
    ...
    def count_by_health(self, health: Optional[EnumAgentHealth] = None) -> int:
