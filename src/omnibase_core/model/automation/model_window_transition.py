...
class ModelWindowTransition(BaseModel):
    ...
    from_window: Optional[str] = Field(None, description="Previous window ID")
    ...
    agents_to_stop: List[str] = Field(
        default_factory=list, description="Agents to stop during transition"
    )
    agents_to_start: List[str] = Field(
        default_factory=list, description="Agents to start in new window"
    )
    agents_to_continue: List[str] = Field(
        default_factory=list, description="Agents continuing across windows"
    )
    ...
    error_message: Optional[str] = Field(None, description="Error if transition failed")

    statistics: Optional[ModelTransitionStatistics] = Field(
        default=None, description="Transition performance statistics"
    )
