...
from omnibase_core.model.automation.model_agent_allocation import (
    EnumAgentStatus, ModelAgentAllocation)
...
class ModelAutomationSchedule(BaseModel):
    ...
    active_agents: List[ModelAgentAllocation] = Field(
        default_factory=list, description="Currently active agent allocations"
    )
    ...
    current_window: Optional[str] = Field(
        None, description="Currently active window ID"
    )
    next_transition: Optional[datetime] = Field(
        None, description="Next window transition time"
    )

    statistics: Optional[ModelScheduleStatistics] = Field(
        default=None, description="Schedule performance statistics"
    )

    def get_current_window(self) -> Optional[ModelOperationalWindow]:
    ...
    def get_available_quota(self) -> int:
        ...
        return max(
            0, self.daily_quota_limit - self.quota_consumed - self.emergency_reserve
        )
