...
class ModelPassageEmbedding(BaseModel):
    ...
    passage_type: str = Field(
        default="text", description="Type of passage (text, code, table, list, etc.)"
    )
    semantic_density: float = Field(
        default=0.0, description="Semantic density score (0.0 to 1.0)"
    )
    contextual_weight: float = Field(
        default=1.0, description="Contextual importance weight within document"
    )
    metadata: Dict[str, Union[str, int, float, bool]] = Field(
        default_factory=dict, description="Additional passage metadata"
    )
...
class ModelDocumentStructure(BaseModel):
    ...
    section_level: int = Field(
        default=0, description="Hierarchical level (0=root, 1=h1, 2=h2, etc.)"
    )
    section_path: List[str] = Field(
        default_factory=list, description="Hierarchical path to this section"
    )
    parent_section: Optional[str] = Field(None, description="Parent section identifier")
    child_sections: List[str] = Field(
        default_factory=list, description="Child section identifiers"
    )
...
class ModelMultiVectorDocument(BaseModel):
    ...
    document_title: Optional[str] = Field(
        None, description="Document title or filename"
    )
    document_url: Optional[str] = Field(None, description="Source URL or file path")
    ...
    document_structure: Optional[ModelDocumentStructure] = Field(
        None, description="Hierarchical document structure information"
    )
    ...
    document_embedding: Optional[List[float]] = Field(
        None, description="Document-level embedding for coarse-grained retrieval"
    )
    document_summary: Optional[str] = Field(
        None, description="Generated document summary"
    )
    ...
    semantic_coherence: float = Field(
        default=0.0, description="Semantic coherence score across passages"
    )
    passage_diversity: float = Field(
        default=0.0, description="Semantic diversity score across passages"
    )
    ...
    content_type: str = Field(
        default="text", description="Content type (text, code, markdown, etc.)"
    )
    tags: List[str] = Field(
        default_factory=list, description="Document tags for filtering"
    )
    metadata: Dict[str, Union[str, int, float, bool]] = Field(
        default_factory=dict, description="Additional document metadata"
    )

    def get_passage_by_id(self, passage_id: str) -> Optional[ModelPassageEmbedding]:
    ...
    def get_context_for_passage(
        self, passage_id: str, context_size: int = 1
