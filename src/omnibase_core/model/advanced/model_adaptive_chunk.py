...
class ModelModelAdaptiveChunkMetadata(BaseModel):
    ...
    density_score: float = Field(
        ...
        ge=0.0,
        le=1.0,
        description="Average content density for this chunk",
    )
    ...
    preserved_entities: list[str] = Field(
        ...
        description="Names of entities preserved in this chunk",
    )
    ...
    boundary_confidence: float = Field(
        ...
        ge=0.0,
        le=1.0,
        description="Confidence in the boundary selection",
    )
    langextract_patterns_detected: list[str] = Field(
        default_factory=list,
        description="LangExtract patterns detected in this chunk",
    )
    intelligence_enhanced: bool = Field(
        default=False,
        description="Whether this chunk used LangExtract intelligence",
    )
