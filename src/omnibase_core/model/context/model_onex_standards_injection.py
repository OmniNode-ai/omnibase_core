...
class ModelOnexStandardsSection(BaseModel):
    ...
    token_estimate: int = Field(
        default=0, ge=0, description="Estimated token count for this section"
    )
    relevance_score: float = Field(
        default=0.0, ge=0.0, le=1.0, description="Relevance score for current request"
    )
...
class ModelCodeDetectionAnalysis(BaseModel):
    ...
    coding_confidence: float = Field(
        ..., ge=0.0, le=1.0, description="Confidence this is a coding task"
    )
    ...
    keyword_score: float = Field(
        default=0.0, ge=0.0, le=1.0, description="Score from coding keywords"
    )
    technical_context_score: float = Field(
        default=0.0, ge=0.0, le=1.0, description="Score from technical context"
    )
    intent_category_score: float = Field(
        default=0.0, ge=0.0, le=1.0, description="Score from intent categorization"
    )
    structure_analysis_score: float = Field(
        default=0.0, ge=0.0, le=1.0, description="Score from request structure"
    )

    # Detected patterns
    detected_keywords: List[str] = Field(
        default_factory=list, description="Coding-related keywords found"
    )
    detected_technologies: List[str] = Field(
        default_factory=list, description="Technologies/frameworks detected"
    )
    detected_onex_terms: List[str] = Field(
        default_factory=list, description="ONEX-specific terms detected"
    )
    ...
    recommended_sections: List[EnumONEXStandardsSection] = Field(
        default_factory=list, description="Recommended standards sections"
    )
...
class ModelOnexStandardsInjection(BaseModel):
    ...
    injected_sections: List[ModelOnexStandardsSection] = Field(
        default_factory=list, description="Standards sections injected"
    )
    ...
    total_standards_tokens: int = Field(
        default=0, ge=0, description="Total token count for standards"
    )

    # Performance metrics
    processing_time_ms: int = Field(
        ..., ge=0, description="Time taken to process standards injection"
    )
    cache_hit: bool = Field(
        default=False, description="Whether standards were loaded from cache"
    )
...
class ModelOnexStandardsSettings(BaseModel):
    ...
    enabled: bool = Field(
        default=True, description="Whether standards injection is enabled"
    )
    ...
    max_sections_per_injection: int = Field(
        default=5, ge=1, le=10, description="Maximum standards sections per injection"
    )
    ...
    cache_standards: bool = Field(
        default=True, description="Whether to cache parsed standards"
    )
    cache_ttl_seconds: int = Field(
        default=3600, ge=60, le=86400, description="Standards cache TTL"
    )

    # Quality settings
    prioritize_critical_rules: bool = Field(
        default=True, description="Always prioritize critical enforcement rules"
    )
    context_aware_selection: bool = Field(
        default=True, description="Select sections based on request context"
    )
    token_budget_enforcement: bool = Field(
        default=True, description="Enforce token budget limits"
    )
...
class ModelOnexStandardsCache(BaseModel):
    ...
    created_at: datetime = Field(
        default_factory=datetime.now, description="When cache was created"
    )
...
class ModelOnexStandardsMetrics(BaseModel):
    ...
    total_requests: int = Field(
        default=0, ge=0, description="Total standards injection requests"
    )
    successful_injections: int = Field(
        default=0, ge=0, description="Successful standards injections"
    )
    failed_injections: int = Field(
        default=0, ge=0, description="Failed standards injections"
    )

    # Detection metrics
    code_detection_accuracy: float = Field(
        default=0.0, ge=0.0, le=1.0, description="Code detection accuracy"
    )
    average_confidence_score: float = Field(
        default=0.0, ge=0.0, le=1.0, description="Average detection confidence"
    )
    ...
    cache_hit_rate: float = Field(
        default=0.0, ge=0.0, le=1.0, description="Standards cache hit rate"
    )

    # Usage metrics
    requests_by_confidence: Dict[str, int] = Field(
        default_factory=dict, description="Requests by confidence level"
    )
    most_injected_sections: Dict[str, int] = Field(
        default_factory=dict, description="Most frequently injected sections"
    )
