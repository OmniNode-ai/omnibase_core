...
class model_HistoricalContext(BaseModel):
    ...
    tools_used: list[str] = Field(
        ...
        description="Tools used in the conversation",
    )
    similarity_score: float = Field(
        ...,
        ge=0.0,
        le=1.0,
        description="Similarity score to current request",
    )
...
class model_ContextInjectionRequest(BaseModel):
    ...
    include_failed_attempts: bool = Field(
        default=False,
        description="Whether to include failed conversation attempts",
    )
    time_window_days: int | None = Field(
        None,
        ge=1,
        le=365,
        description="Limit context to conversations within N days",
    )
...
class model_ContextInjectionResponse(BaseModel):
    ...
    extracted_keywords: list[str] = Field(
        default_factory=list,
        description="Keywords extracted from request",
    )
    ...
    historical_contexts: list[model_HistoricalContext] = Field(
        default_factory=list,
        description="Relevant historical conversations",
    )
    ...
    confidence_score: float = Field(
        ...
        ge=0.0,
        le=1.0,
        description="Confidence in context relevance",
    )
    ...
    context_items_injected: int = Field(
        ...,
        ge=0,
        description="Context items actually injected",
    )
    processing_time_ms: int = Field(
        ...,
        ge=0,
        description="Time taken to process injection",
    )

    # Enhanced prompt
    enhanced_context: str | None = Field(
        None,
        description="Enhanced context for Claude's working memory",
    )
...
class model_ContextEnhancementPrompt(BaseModel):
    ...
    context_items_count: int = Field(
        ...,
        ge=0,
        description="Number of context items included",
    )
    enhancement_timestamp: datetime = Field(
        default_factory=datetime.now,
        description="When enhancement was applied",
    )
...
class model_ContextInjectionSettings(BaseModel):
    ...
    default_mode: EnumContextInjectionMode = Field(
        default=EnumContextInjectionMode.AUTOMATIC,
        description="Default injection mode",
    )
    max_context_items: int = Field(
        default=5,
        ge=1,
        le=20,
        description="Default maximum context items",
    )
    relevance_threshold: float = Field(
        default=0.3,
        ge=0.0,
    ...
    time_window_days: int = Field(
        ...
        ge=1,
        le=365,
        description="Default time window for context searches",
    )
    ...
    cache_results: bool = Field(
        default=True,
        description="Whether to cache context results",
    )
    ...
    require_successful_outcomes: bool = Field(
        default=False,
        description="Only include contexts from successful conversations",
    )
    prioritize_recent: bool = Field(
        default=True,
        description="Give higher weight to recent conversations",
    )
    domain_specific_matching: bool = Field(
        default=True,
        description="Use domain-specific keyword matching",
    )
...
class model_ContextInjectionMetrics(BaseModel):
    ...
    total_requests: int = Field(
        default=0,
        ge=0,
        description="Total injection requests processed",
    )
    successful_injections: int = Field(
        default=0,
        ge=0,
        description="Successful context injections",
    )
    failed_injections: int = Field(
        default=0,
        ge=0,
        description="Failed context injections",
    )
    ...
    max_processing_time_ms: int = Field(
        default=0,
        ge=0,
        description="Maximum processing time recorded",
    )
    min_processing_time_ms: int = Field(
        default=0,
        ge=0,
        description="Minimum processing time recorded",
    )
    ...
    average_context_items: float = Field(
        default=0.0,
        ge=0.0,
        description="Average context items injected",
    )

    # Usage metrics
    requests_by_mode: dict[str, int] = Field(
        default_factory=dict,
        description="Request counts by injection mode",
    )
    requests_by_relevance: dict[str, int] = Field(
        default_factory=dict,
        description="Request counts by relevance level",
    )
