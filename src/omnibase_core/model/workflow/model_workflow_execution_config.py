...
class ModelWorkflowExecutionConfig(BaseModel):
    ...
    parallel_workers: int = Field(
        default=1, description="Number of parallel workers", ge=1, le=100
    )
    priority_weight: float = Field(
        default=1.0, description="Execution priority weight", ge=0.1, le=10.0
    )

    # Performance characteristics for distributed execution
    target_speed_factor: Optional[float] = Field(
        None,
        description="Target execution speed factor (1.0 = baseline)",
        gt=0.0,
        le=10.0,
    )
    accuracy_requirement: Optional[float] = Field(
        None, description="Required accuracy level (0.0-1.0)", ge=0.0, le=1.0
    )
    cost_budget_per_execution: Optional[float] = Field(
        None, description="Cost budget per execution in credits", ge=0.0
    )
    max_load_factor: Optional[float] = Field(
        None, description="Maximum system load factor (0.0-1.0)", ge=0.0, le=1.0
    )

    # Resource constraints
    memory_limit_mb: Optional[int] = Field(
        None, description="Memory limit in megabytes", gt=0, le=65536  # 64GB max
    )
    cpu_limit_cores: Optional[float] = Field(
        None, description="CPU core limit", gt=0.0, le=64.0
    )
    ...
    enable_retry: bool = Field(
        default=True, description="Enable automatic retry on failure"
    )
    ...
    checkpoint_interval_seconds: Optional[int] = Field(
        None, description="Checkpoint interval in seconds", gt=0
    )

    # Quality of service
    consistency_level: str = Field(
        default="eventual", description="Consistency level for distributed execution"
    )
    ...
    def validate_backoff_strategy(cls, v):
        ...
        if v not in valid_strategies:
            raise ValueError(
                f"retry_backoff_strategy must be one of {valid_strategies}"
            )
        return v
    ...
    def validate_checkpoint_interval(cls, v, info):
        ...
        if v is not None and hasattr(info, "data") and "timeout_seconds" in info.data:
            if v >= info.data["timeout_seconds"]:
                raise ValueError(
                    "checkpoint_interval_seconds must be less than timeout_seconds"
                )
        return v
