...
class ModelActiveTaskData(BaseModel):
    ...
    ticket: Optional[ModelWorkTicket] = Field(
        default=None, description="Work ticket associated with this task"
    )
    debug_context: Optional[ModelDebugContext] = Field(
        default=None, description="Debug intelligence context for the task"
    )
    ...
    agent_id: Optional[str] = Field(default=None, description="ID of assigned agent")
    execution_log: List[str] = Field(
        default_factory=list, description="Log of execution steps and events"
    )
