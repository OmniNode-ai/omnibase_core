...
class ModelServiceType(BaseModel):
    ...
    default_port: Optional[int] = Field(
        None, description="Default port for this service type", ge=1, le=65535
    )

    supports_health_checks: bool = Field(
        True, description="Whether this service type supports health checking"
    )

    supports_load_balancing: bool = Field(
        True, description="Whether this service type supports load balancing"
    )

    connection_pooling: bool = Field(
        False, description="Whether this service type benefits from connection pooling"
    )
    ...
    metadata: Optional[ModelGenericMetadata] = Field(
        default_factory=dict, description="Additional service type configuration"
    )
    ...
    def validate_custom_name_consistency(cls, v, info):
        ...
        if hasattr(info, "data") and info.data:
            ...
            if type_category == ServiceTypeCategory.CUSTOM and not v:
                raise ValueError(
                    "custom_type_name is required when type_category is CUSTOM"
                )
        return v
