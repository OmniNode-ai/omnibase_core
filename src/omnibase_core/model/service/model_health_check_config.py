...
class ModelHealthCheckConfig(BaseModel):
    ...
    interval_seconds: int = Field(
        30, description="Health check interval in seconds", ge=1, le=300
    )
    timeout_seconds: int = Field(
        10, description="Health check timeout in seconds", ge=1, le=60
    )
    retries: int = Field(3, description="Number of health check retries", ge=1, le=10)
    start_period_seconds: int = Field(
        60, description="Grace period before health checks start", ge=0, le=300
    )
