...
class ModelServiceHealth(BaseModel):
    ...
    def is_healthy(self) -> bool:
        ...
        return self.status == ServiceHealthStatus.REACHABLE
    ...
    def requires_attention(self) -> bool:
        ...
        return (
            ...
            or (
                self.response_time_ms and self.response_time_ms > 30000
            )  # 30 second threshold for services
        )
    ...
    def calculate_reliability_score(self) -> float:
        ...
        if self.status == ServiceHealthStatus.ERROR:
            base_score = 0.0
        elif self.status == ServiceHealthStatus.TIMEOUT:
            base_score *= 0.3
        elif self.status == ServiceHealthStatus.DEGRADED:
            base_score *= 0.5
    ...
    def get_business_impact(self) -> "ModelBusinessImpact":
        ...
        return ModelBusinessImpact(
            ...
            downtime_minutes=(
                float(self.consecutive_failures * 5) if self.is_unhealthy() else None
            ),
    ...
    def create_healthy(
        ...
        return cls(
            ...
            status=ServiceHealthStatus.REACHABLE,
    ...
    def create_timeout(
        ...
        return cls(
            service_name=service_name,
            service_type=ServiceType(service_type),
            status=ServiceHealthStatus.TIMEOUT,
            connection_string=connection_string,
