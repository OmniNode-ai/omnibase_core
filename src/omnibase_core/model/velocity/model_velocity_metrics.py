...
from omnibase_core.model.ai_workflows.model_ai_execution_metrics import (
    EnumMetricType, EnumMetricUnit, ModelMetricValue)
...
class ModelVelocityMetrics(BaseModel):
    ...
    lines_of_code_changed: int = Field(
        default=0, ge=0, description="Number of lines of code modified"
    )
    files_modified: int = Field(
        default=0, ge=0, description="Number of files that were changed"
    )
    tools_used: List[ModelToolType] = Field(
        default_factory=list, description="List of ONEX tools used during the task"
    )
    ...
    validation_passes: int = Field(
        default=0, ge=0, description="Number of validation checks that passed"
    )

    # Additional standardized metrics
    additional_metrics: List[ModelMetricValue] = Field(
        default_factory=list, description="Additional metrics using ONEX metric format"
    )
