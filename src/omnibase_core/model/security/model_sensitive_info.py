...
from omnibase_core.model.security.model_detection_result import (
    EnumDetectionMethod, EnumDetectionType, EnumSensitivityLevel)
...
class ModelDetectionPattern(BaseModel):
    ...
    pattern_regex: Optional[str] = Field(
        default=None, description="Regular expression pattern"
    )
    ...
    context_window_size: int = Field(
        default=50, ge=0, description="Size of context window around matches"
    )

    enabled: bool = Field(
        default=True, description="Whether this pattern is currently enabled"
    )
    ...
    false_positive_patterns: List[str] = Field(
        default_factory=list, description="Patterns to exclude as false positives"
    )

    examples: List[str] = Field(
        default_factory=list, description="Example strings this pattern should match"
    )

    description: Optional[str] = Field(
        default=None, description="Description of what this pattern detects"
    )
...
class ModelDetectionRuleSet(BaseModel):
    ...
    performance_target_docs_per_minute: Optional[int] = Field(
...
class ModelDetectionConfiguration(BaseModel):
    ...
    enabled_rulesets: List[str] = Field(
        default_factory=list, description="IDs of enabled rulesets"
    )

    global_confidence_threshold: float = Field(
        default=0.7, ge=0.0, le=1.0, description="Global minimum confidence threshold"
    )

    max_document_size_mb: int = Field(
        default=10, ge=1, description="Maximum document size to process"
    )

    enable_false_positive_reduction: bool = Field(
        default=True, description="Whether to apply false positive reduction"
    )

    enable_context_analysis: bool = Field(
        default=True, description="Whether to use context analysis"
    )

    enable_ml_detection: bool = Field(
        default=True, description="Whether to use ML-based detection"
    )

    parallel_processing_workers: int = Field(
        default=4, ge=1, description="Number of parallel processing workers"
    )

    audit_logging_enabled: bool = Field(
        default=True, description="Whether to log all detections for audit"
    )

    redaction_character: str = Field(
        default="*", description="Character to use for redaction"
    )
