...
class ModelApprovalRequirements(BaseModel):
    ...
    min_approvals: int = Field(
        1, description="Minimum number of approvals required", ge=0, le=10
    )
    ...
    escalation_enabled: bool = Field(
        False, description="Whether escalation is enabled after timeout"
    )
    ...
    approval_message_template: Optional[str] = Field(
        None, description="Template for approval request messages"
    )
