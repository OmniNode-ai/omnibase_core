...
This module provides backward compatibility imports for secret management
...
- ModelKafkaSecureConfig → ModelKafkaSecureConfig (model_kafka_secure_config.py)
...
# Provide backward compatibility for the original enum
class SecretBackendEnum:
    """
    Legacy enum compatibility layer.
...
# Provide backward compatibility for SecretManager
class SecretManager:
    """
    Legacy SecretManager compatibility layer.
    ...
    def get_kafka_config(self):
        """Get Kafka configuration (legacy method)."""
        return self._manager.get_kafka_config()
...
def validate_secret_configuration(config_type: str, **kwargs) -> dict:
    ...
    Args:
        config_type: Type of configuration ('kafka', 'database', 'backend')
