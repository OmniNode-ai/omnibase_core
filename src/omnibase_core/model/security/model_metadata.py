...
class ModelSignatureRequirements(BaseModel):
    ...
    minimum_signatures: int = Field(
        default=1, description="Minimum number of signatures required"
    )
...
class ModelVerificationResult(BaseModel):
    ...
    trust_level: str = Field(
        default="untrusted", description="Trust level of verification"
    )
...
class ModelComplianceStatus(BaseModel):
    ...
    audit_trail_complete: bool = Field(
        default=False, description="Whether audit trail is complete"
    )
...
class ModelVerificationDetails(BaseModel):
    ...
    certificate_validation: Dict[str, Optional[str]] = Field(
        default_factory=dict, description="Certificate validation details"
    )
    ...
    security_level: str = Field(
        default="basic", description="Security level assessment"
    )
    compliance_level: str = Field(
        default="basic", description="Compliance level assessment"
    )
    ...
    verification_time_breakdown: Dict[str, float] = Field(
        default_factory=dict, description="Time breakdown for verification steps"
    )
    performance_optimizations: Dict[str, bool] = Field(
        default_factory=dict, description="Performance optimization flags"
    )
...
class ModelTimeBreakdown(BaseModel):
    ...
    total_time_ms: float = Field(
        default=0.0, description="Total verification time in milliseconds"
    )
    certificate_validation_ms: float = Field(
        default=0.0, description="Certificate validation time in milliseconds"
    )
    signature_verification_ms: float = Field(
        default=0.0, description="Signature verification time in milliseconds"
    )
    chain_validation_ms: float = Field(
        default=0.0, description="Chain validation time in milliseconds"
    )
    cache_lookup_ms: float = Field(
        default=0.0, description="Cache lookup time in milliseconds"
    )
    network_operations_ms: float = Field(
        default=0.0, description="Network operations time in milliseconds"
    )
...
class ModelPerformanceOptimizations(BaseModel):
    ...
    certificate_cached: bool = Field(
        default=False, description="Whether certificate was cached"
    )
    signature_cached: bool = Field(
        default=False, description="Whether signature was cached"
    )
    trusted_node: bool = Field(
        default=False, description="Whether node is in trusted list"
    )
    parallel_verification: bool = Field(
        default=False, description="Whether parallel verification was used"
    )
    fast_path: bool = Field(
        default=False, description="Whether fast verification path was used"
    )
...
class ModelVerificationMetrics(BaseModel):
    ...
    average_verification_time_ms: float = Field(
        default=0.0, description="Average verification time in milliseconds"
    )
    fastest_verification_ms: float = Field(
        default=0.0, description="Fastest verification time in milliseconds"
    )
    slowest_verification_ms: float = Field(
        default=0.0, description="Slowest verification time in milliseconds"
    )
...
class ModelCacheSizes(BaseModel):
    ...
    signature_cache_size: int = Field(
        default=0, description="Number of cached signature results"
    )
    certificate_cache_size: int = Field(
        default=0, description="Number of cached certificate validations"
    )
    content_hash_cache_size: int = Field(
        default=0, description="Number of cached content hashes"
    )
...
class ModelTrustedNodesInfo(BaseModel):
    ...
    total_trusted_nodes: int = Field(
        default=0, description="Total number of trusted nodes"
    )
    high_trust_nodes: int = Field(default=0, description="Number of high trust nodes")
    trusted_node_ids: List[str] = Field(
        default_factory=list, description="List of trusted node identifiers"
    )
    high_trust_node_ids: List[str] = Field(
        default_factory=list, description="List of high trust node identifiers"
    )
...
class ModelPerformanceSettings(BaseModel):
    ...
    max_verification_time_ms: int = Field(
        default=15000, description="Maximum verification time in milliseconds"
    )
    parallel_verification_enabled: bool = Field(
        default=True, description="Whether parallel verification is enabled"
    )
    enable_caching: bool = Field(default=True, description="Whether caching is enabled")
    max_parallel_verifications: int = Field(
        default=10, description="Maximum parallel verifications"
    )
