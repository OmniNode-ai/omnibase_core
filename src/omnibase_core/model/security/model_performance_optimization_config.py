...
class ModelPerformanceOptimizationConfig(BaseModel):
    ...
    cache_ttl_seconds: int = Field(
        300, description="Cache time-to-live in seconds", ge=0, le=3600
    )

    connection_pooling: bool = Field(
        False, description="Whether connection pooling is enabled"
    )

    max_connections: int = Field(
        10, description="Maximum number of connections in pool", ge=1, le=100
    )
