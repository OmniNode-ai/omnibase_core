...
class ModelNotificationSettings(BaseModel):
    ...
    email_on_completion: bool = Field(
        False, description="Send email on execution completion"
    )
    ...
    webhook_headers: Dict[str, str] = Field(
        default_factory=dict, description="Custom headers for webhook"
    )
    webhook_retry_count: int = Field(
        3, description="Number of webhook retry attempts", ge=0, le=5
    )
    ...
    rate_limit_per_hour: Optional[int] = Field(
        None, description="Maximum notifications per hour", ge=1
    )
    aggregate_notifications: bool = Field(
        True, description="Aggregate multiple notifications"
    )
    aggregation_window_minutes: int = Field(
        5, description="Window for notification aggregation", ge=1, le=60
    )
