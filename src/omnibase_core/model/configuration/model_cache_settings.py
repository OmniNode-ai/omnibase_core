...
class ModelCacheSettings(BaseModel):
    ...
    max_ttl_seconds: Optional[int] = Field(3600, description="Maximum TTL in seconds")

    # Size limits
    max_size_mb: Optional[int] = Field(100, description="Maximum cache size in MB")
    max_entries: Optional[int] = Field(1000, description="Maximum number of entries")
    ...
    # Cache key settings
    key_prefix: Optional[str] = Field(None, description="Cache key prefix")
    ...
    def get_effective_ttl(self, requested_ttl: Optional[int] = None) -> int:
