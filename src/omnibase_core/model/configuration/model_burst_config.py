...
class ModelBurstConfig(BaseModel):
    ...
    def get_adaptive_burst_size(self, historical_peaks: list, base_limit: int) -> int:
    ...
    @classmethod
    def create_conservative_burst(cls) -> "ModelBurstConfig":
        """Create conservative burst configuration"""
        return cls(
            burst_detection_enabled=True,
            burst_capacity_multiplier=1.5,
            burst_duration_seconds=15,
            burst_threshold_multiplier=1.2,
            burst_cooldown_seconds=120,
            allow_burst_overflow=False,
            adaptive_burst_sizing=False,
        )

    @classmethod
    def create_aggressive_burst(cls) -> "ModelBurstConfig":
        """Create aggressive burst configuration for high-traffic applications"""
        return cls(
            burst_detection_enabled=True,
            ...
            allow_burst_overflow=True,
            overflow_penalty_multiplier=1.5,
            adaptive_burst_sizing=True,
        )

    @classmethod
    def create_predictive_burst(cls) -> "ModelBurstConfig":
        """Create predictive burst configuration with advanced features"""
        return cls(
            burst_detection_enabled=True,
            burst_capacity_multiplier=2.5,
            burst_duration_seconds=45,
            burst_threshold_multiplier=1.8,
            burst_cooldown_seconds=60,
            adaptive_burst_sizing=True,
            burst_prediction_enabled=True,
            burst_warning_threshold=0.7,
            allow_burst_overflow=True,
        )

    @classmethod
    def create_disabled_burst(cls) -> "ModelBurstConfig":
        """Create configuration with burst handling disabled"""
        return cls(
