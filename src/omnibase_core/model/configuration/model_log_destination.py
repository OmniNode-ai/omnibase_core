...
class ModelLogDestinationConfig(BaseModel):
    ...
    max_file_size_mb: Optional[int] = Field(
        None, description="Maximum file size in MB before rotation", ge=1
    )
    max_files: Optional[int] = Field(
        None, description="Maximum number of rotated files to keep", ge=1
    )
    file_permissions: Optional[str] = Field(
        None, description="File permissions (octal string like '0644')"
    )
    compress_rotated: bool = Field(False, description="Compress rotated log files")

    # Network destination options
    protocol: Optional[str] = Field(
        None, description="Network protocol", pattern="^(tcp|udp|http|https)$"
    )
    timeout_ms: Optional[int] = Field(
        None, description="Connection timeout in milliseconds", ge=100
    )
    ...
    table_name: Optional[str] = Field(None, description="Database table name for logs")
    batch_insert: bool = Field(
        True, description="Use batch inserts for better performance"
    )
    create_table_if_missing: bool = Field(
        False, description="Create log table if it doesn't exist"
    )

    # Formatting options
    include_timestamp: bool = Field(True, description="Include timestamp in output")
    timestamp_format: str = Field(
        "%Y-%m-%d %H:%M:%S.%f", description="Timestamp format string"
    )
    ...
    custom_handler_class: Optional[str] = Field(
        None, description="Fully qualified class name for custom handler"
    )
    custom_handler_config: Optional[Dict[str, str]] = Field(
        None, description="Configuration for custom handler"
    )
...
class ModelLogDestination(BaseModel):
    ...
    file_path: Optional[str] = Field(
        None, description="File path for file destinations"
    )
    host: Optional[str] = Field(None, description="Host for network destinations")
    port: Optional[int] = Field(
        None, description="Port for network destinations", ge=1, le=65535
    )
    connection_string: Optional[str] = Field(
        None, description="Connection string for database destinations"
    )
    ...
    buffer_size: int = Field(
        default=1000, description="Buffer size for batched logging", ge=1
    )
    flush_interval_ms: int = Field(
        default=5000, description="Flush interval in milliseconds", ge=100
    )
    ...
    def create_network(cls, name: str, host: str, port: int) -> "ModelLogDestination":
        ...
        return cls(
            destination_type="network", destination_name=name, host=host, port=port
        )
