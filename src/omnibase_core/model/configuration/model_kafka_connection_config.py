...
class ModelKafkaConnectionConfig(BaseModel):
    ...
    timeout_ms: int = Field(
        5000, description="Connection timeout in milliseconds", ge=1000, le=300000
    )
    ...
    sasl_username: Optional[str] = Field(
        None, description="SASL username for authentication", max_length=100
    )
    sasl_password: Optional[SecretStr] = Field(
        None, description="SASL password for authentication (secured)"
    )
    ssl_keyfile: Optional[str] = Field(
        None, description="Path to SSL key file", max_length=500
    )
    ssl_certfile: Optional[str] = Field(
        None, description="Path to SSL certificate file", max_length=500
    )
    ssl_cafile: Optional[str] = Field(
        None, description="Path to SSL CA file", max_length=500
    )
        ...
        for server in servers:
            ...
            if ":" in server:
                ...
                try:
                    port = int(port_str)
                    if not (1 <= port <= 65535):
                        raise ValueError(f"Invalid port number in server: {server}")
                except ValueError:
                    raise ValueError(f"Invalid port format in server: {server}")
            else:
                host = server

            # Host validation
            if not re.match(r"^[a-zA-Z0-9\-\.]+$", host):
                raise ValueError(f"Invalid host format in server: {server}")

            validated_servers.append(server)

        if not validated_servers:
            raise ValueError("No valid bootstrap servers found")
    ...
    def validate_ssl_files(cls, v: Optional[str]) -> Optional[str]:
        ...
        if v is not None:
            ...
            if not re.match(r"^[a-zA-Z0-9\-\._/\\:]+$", v):
                raise ValueError(f"Invalid SSL file path format: {v}")
        return v
    ...
    def parse_topic_name(self, full_topic: str) -> Tuple[Optional[str], str]:
