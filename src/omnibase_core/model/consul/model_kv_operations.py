...
class ModelNodeMetadata(BaseModel):
    ...
    status: str = Field(
        "active",
        description="Node status: active, deprecated, archived",
    )
    ...
    endpoints: dict[str, str] = Field(
        ...
        description="Service endpoints (service, health, metrics)",
    )
...
class ModelSyncResult(BaseModel):
    ...
    modify_index: int | None = Field(None, description="New modify index after sync")
    conflict_detected: bool = Field(
        False,
        description="Whether a conflict was detected",
    )
    conflict_resolution: EnumConflictResolutionStrategy | None = Field(
        None,
    ...
    local_hash: str | None = Field(None, description="Hash of local data")
    remote_hash: str | None = Field(None, description="Hash of remote data")
...
class ModelKVChangeEvent(BaseModel):
    ...
    datacenter: str = Field(
        "dc1",
        description="Consul datacenter where change occurred",
    )
...
class ModelKVSyncConfig(BaseModel):
    ...
    require_signature: bool = Field(
        True,
        description="Require digital signatures on writes",
    )
    hash_algorithm: str = Field(
        "sha256",
        description="Hash algorithm for integrity checks",
    )
...
class ModelKVBatch(BaseModel):
    ...
    operations: list[dict[str, Any]] = Field(
        default_factory=list,
        description="List of KV operations in the batch",
    )
...
class ModelKVWatch(BaseModel):
    ...
    datacenter: str | None = Field(None, description="Specific datacenter to watch")
...
class ModelKVWatchResult(BaseModel):
    ...
    changes: list[ModelKVChangeEvent] = Field(
        default_factory=list,
        description="List of KV changes detected",
    )
    ...
    error_message: str | None = Field(
        None,
        description="Error message if watch failed",
    )
