...
class ModelAIServiceConfiguration(BaseModel):
    ...
    connection_config: Dict[str, Any] = Field(
        default_factory=dict, description="Service connection configuration"
    )
...
class ModelDocumentProcessingConfiguration(BaseModel):
    ...
    supported_formats: List[str] = Field(
        default=["pdf", "docx", "html", "txt"], description="Supported document formats"
    )
    max_file_size_mb: int = Field(default=100, description="Maximum file size in MB")
    batch_size: int = Field(
        default=50, description="Number of documents to process in a batch"
    )
    extraction_config: Dict[str, Any] = Field(
        default_factory=dict, description="Text extraction configuration"
    )
    chunking_config: Dict[str, Any] = Field(
        default_factory=dict, description="Text chunking configuration"
    )
    enable_parallel_processing: bool = Field(
        default=True, description="Enable parallel document processing"
    )
    ...
    def validate_formats(cls, v):
        ...
        for format_type in v:
            if format_type not in allowed_formats:
                raise ValueError(
                    f"Format {format_type} not in allowed formats {allowed_formats}"
                )
        return v
...
class ModelEmbeddingConfiguration(BaseModel):
    ...
    cache_embeddings: bool = Field(
        default=True, description="Cache generated embeddings"
    )
    ...
    def validate_dimension(cls, v):
        ...
        if v not in allowed_dimensions:
            raise ValueError(f"Embedding dimension must be one of {allowed_dimensions}")
        return v
...
class ModelVectorDatabaseConfiguration(BaseModel):
    ...
    index_config: Dict[str, Any] = Field(
        default_factory=dict, description="Vector index configuration"
    )
    replication_factor: int = Field(
        default=1, description="Number of replicas for the collection"
    )
    shard_number: int = Field(
        default=1, description="Number of shards for the collection"
    )
    enable_payload_index: bool = Field(
        default=True, description="Enable indexing of payload fields"
    )
    optimization_config: Dict[str, Any] = Field(
        default_factory=dict, description="Index optimization configuration"
    )
...
class ModelSearchConfiguration(BaseModel):
    ...
    enable_hybrid_search: bool = Field(
        default=False, description="Enable hybrid search with text matching"
    )
    metadata_filters: Dict[str, Any] = Field(
        default_factory=dict, description="Metadata filters for search"
    )
    result_ranking_config: Dict[str, Any] = Field(
        default_factory=dict, description="Result ranking configuration"
    )
...
class ModelPerformanceConfiguration(BaseModel):
    ...
    max_processing_time_minutes: int = Field(
        default=30, description="Maximum processing time in minutes"
    )
    memory_limit_gb: int = Field(default=16, description="Memory limit in GB")
    cpu_cores: Optional[int] = Field(None, description="Number of CPU cores to use")
    ...
    performance_thresholds: Dict[str, float] = Field(
        default_factory=dict, description="Performance thresholds"
    )
...
class ModelValidationConfiguration(BaseModel):
    ...
    enable_performance_validation: bool = Field(
        default=True, description="Enable performance validation"
    )
    ...
    quality_metrics: List[str] = Field(
        default_factory=list, description="Quality metrics to track"
    )
    failure_handling: str = Field(
        default="retry", description="How to handle validation failures"
    )
    ...
    def validate_failure_handling(cls, v):
        ...
        if v not in allowed_handling:
            raise ValueError(f"Failure handling must be one of {allowed_handling}")
        return v
...
class ModelAIWorkflowConfiguration(BaseModel):
    ...
    workflow_version: str = Field(
        default="1.0.0", description="Version of the workflow"
    )
    ...
    document_processing: Optional[ModelDocumentProcessingConfiguration] = Field(
        None, description="Document processing configuration"
    )
    embedding: Optional[ModelEmbeddingConfiguration] = Field(
        None, description="Embedding generation configuration"
    )
    vector_database: Optional[ModelVectorDatabaseConfiguration] = Field(
        None, description="Vector database configuration"
    )
    search: Optional[ModelSearchConfiguration] = Field(
    ...
    custom_parameters: Dict[str, Any] = Field(
        default_factory=dict, description="Custom workflow parameters"
    )
    feature_flags: Dict[str, bool] = Field(
        default_factory=dict, description="Feature flags for the workflow"
    )
    ...
    ) -> Optional[ModelAIServiceConfiguration]:
