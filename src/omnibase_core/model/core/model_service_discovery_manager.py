...
class ModelServiceDiscoveryManager(BaseModel):
    ...
    consul_enabled: bool = Field(
        default=True, description="Whether Consul service discovery is enabled"
    )

    consul_datacenter: str = Field(
        default="dc1", description="Consul datacenter for service discovery"
    )

    consul_timeout: int = Field(
        default=10, description="Consul connection timeout in seconds"
    )

    static_config: Dict[str, object] = Field(
        default_factory=dict, description="Static service configuration mappings"
    )

    service_cache_enabled: bool = Field(
        default=True, description="Whether to cache resolved services"
    )
    ...
    retry_attempts: int = Field(
        default=3, description="Number of retry attempts for failed service discovery"
    )

    health_check_interval: int = Field(
        default=30, description="Service health check interval in seconds"
    )
