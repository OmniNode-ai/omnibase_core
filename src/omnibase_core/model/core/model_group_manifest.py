...
class ModelGroupServiceEndpoint(BaseModel):
    ...
    delegation_target: Optional[str] = Field(
        default=None, description="Tool to delegate requests to"
    )
...
class ModelGroupServiceConfiguration(BaseModel):
    ...
    default_port: Optional[int] = Field(
        default=None, description="Default HTTP port for the group service"
    )
    http_endpoints: List[ModelGroupServiceEndpoint] = Field(
        default_factory=list, description="HTTP endpoints provided by group"
    )
    websocket_endpoints: List[ModelGroupServiceEndpoint] = Field(
        default_factory=list, description="WebSocket endpoints provided by group"
    )
...
class ModelGroupTool(BaseModel):
    ...
    capabilities: List[str] = Field(
        default_factory=list, description="Tool capabilities list"
    )
...
class ModelGroupManifest(BaseModel):
    ...
    shared_dependencies: List[ModelGroupDependency] = Field(
        default_factory=list, description="Group-wide dependencies"
    )
    shared_protocols: List[str] = Field(
        default_factory=list, description="Shared protocol interfaces"
    )
    shared_models: List[str] = Field(
        default_factory=list, description="Shared Pydantic models"
    )

    # === SERVICE CONFIGURATION ===
    service_configuration: Optional[ModelGroupServiceConfiguration] = Field(
        default=None, description="HTTP service configuration if applicable"
    )
    ...
    security_requirements: Dict[str, str] = Field(
        default_factory=dict, description="Specific security requirements"
    )

    # === DEPLOYMENT CONFIGURATION ===
    deployment_configuration: Dict[str, str] = Field(
        default_factory=dict, description="Deployment-specific settings"
    )
    docker_configuration: Optional[Dict[str, str]] = Field(
        default=None, description="Docker deployment configuration"
    )

    # === M1 INTEGRATION ===
    m1_compliance: bool = Field(
        default=True, description="Whether group follows M1 standards"
    )
    envelope_pattern_enabled: bool = Field(
        default=True, description="Whether M1 envelope/reply pattern is used"
    )
    ...
    def validate_active_tools_count(
        cls, v: int, values: Dict[str, Union[int, str, ModelSemVer]]
    ) -> int:
        ...
        if not isinstance(total, int):
            total = 0
        if v > total:
            raise ValueError("active_tools cannot exceed total_tools")
        return v
    ...
    def validate_tools_list(cls, v: List[ModelGroupTool]) -> List[ModelGroupTool]:
        ...
        if len(tool_names) != len(set(tool_names)):
            raise ValueError("Duplicate tool names found in tools list")
    ...
    def get_tool_by_name(self, tool_name: str) -> Optional[ModelGroupTool]:
        """Get tool by name."""
