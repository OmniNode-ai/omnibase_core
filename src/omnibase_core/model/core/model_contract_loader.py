...
from omnibase_core.core.models.model_contract_reference import \
...
class ModelContractLoader(BaseModel):
    ...
    cache_enabled: bool = Field(
        default=True, description="Whether contract caching is enabled"
    )
    contract_cache: Dict[str, ModelContractCache] = Field(
        default_factory=dict, description="Contract cache storage"
    )
    ...
    loaded_contracts: Dict[str, ModelContractContent] = Field(
        default_factory=dict, description="Successfully loaded contracts"
    )
    resolved_references: Dict[str, ModelContractReference] = Field(
        default_factory=dict, description="Resolved contract references"
    )
