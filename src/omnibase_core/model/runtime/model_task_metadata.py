...
class ModelTaskMetadata(BaseModel):
    ...
    priority_reason: str = Field(
        default="",
        description="Reason for priority assignment",
    )

    source_system: str = Field(
        default="unknown",
        description="System that created the task",
    )
    ...
    business_context: str = Field(
        default="",
        description="Business context for the task",
    )

    tags: dict[str, str] = Field(
        default_factory=dict,
        description="String key-value tags for categorization",
    )
