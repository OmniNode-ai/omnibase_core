...
class ModelQueueManagerConfig(BaseModel):
    ...
    max_batch_size: int = Field(
        50, description="Maximum tasks to fetch per poll", ge=1, le=1000
    )
    retry_failed_after_hours: int = Field(
        24, description="Hours before retrying failed tasks", ge=1
    )
...
class ModelDramatiqWorkerConfig(BaseModel):
    ...
    queue_name: Optional[str] = Field("default", description="Queue to consume from")
...
class ModelDramatiqMiddlewareConfig(BaseModel):
    ...
    age_limit_ms: int = Field(
        3600000, description="Max message age in milliseconds", ge=0
    )
    time_limit_ms: int = Field(
        600000, description="Max execution time in milliseconds", ge=0
    )
...
class ModelDramatiqConfig(BaseModel):
    ...
    workers: List[ModelDramatiqWorkerConfig] = Field(
        default_factory=list, description="Worker configurations"
    )
...
class ModelMonitoringConfig(BaseModel):
    ...
    health_check_interval: int = Field(
        30, description="Seconds between health checks", ge=1
    )
    worker_restart_threshold: int = Field(
        3, description="Failed checks before restart", ge=1
    )
...
class ModelTaskQueueConfig(BaseModel):
    ...
    dramatiq: ModelDramatiqConfig = Field(
        default_factory=ModelDramatiqConfig, description="Dramatiq configuration"
    )
