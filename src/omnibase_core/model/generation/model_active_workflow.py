...
from omnibase_core.model.ai_workflows.model_workflow_state import \
    EnumWorkflowStatus
...
class ModelActiveWorkflow(BaseModel):
    ...
    contract_path: Optional[str] = Field(
        default=None, description="Path to contract file if applicable"
    )
    output_path: Optional[str] = Field(
        default=None, description="Output directory path if applicable"
    )
    tool_name: Optional[str] = Field(
        default=None, description="Name of tool being generated"
    )
    ...
    result_data: Optional[str] = Field(
        default=None, description="Serialized result data as JSON string"
    )
