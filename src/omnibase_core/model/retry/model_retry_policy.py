...
class ModelRetryPolicy(BaseModel):
    ...
    strategy: EnumRetryStrategy = Field(
        EnumRetryStrategy.EXPONENTIAL_BACKOFF, description="Retry strategy to use"
    )
    ...
    backoff_multiplier: float = Field(
        2.0, description="Multiplier for exponential backoff"
    )
    ...
    custom_delays: Optional[List[float]] = Field(
        None, description="Custom delay sequence in seconds (for CUSTOM strategy)"
    )

    # Circuit breaker integration
    circuit_breaker_enabled: bool = Field(
        True, description="Whether to integrate with circuit breaker"
    )

    # Timeout configuration
    operation_timeout: float = Field(
        30.0, description="Timeout for individual operation attempts"
    )
    total_timeout: float = Field(
        300.0, description="Total timeout including all retry attempts"
    )
