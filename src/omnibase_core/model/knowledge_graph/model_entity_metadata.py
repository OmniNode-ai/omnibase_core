...
from omnibase_core.model.knowledge_graph.model_code_location import \
    ModelCodeLocation
from omnibase_core.model.knowledge_graph.model_documentation_reference import \
...
class ModelEntityMetadata(BaseModel):
    ...
    code_location: Optional[ModelCodeLocation] = Field(
        None, description="Code location details"
    )

    # Documentation metadata
    documentation_refs: List[ModelDocumentationReference] = Field(
        default_factory=list, description="References to related documentation"
    )

    # Semantic metadata
    semantic_similarity_threshold: float = Field(
        0.8, description="Threshold for semantic matching"
    )
    ...
    relationship_count: int = Field(
        0, description="Number of relationships this entity has"
    )
