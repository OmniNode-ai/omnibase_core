...
class ModelWaitTimeMetrics(BaseModel):
    ...
    max_estimated_wait_minutes: float = Field(
        description="Maximum estimated wait time",
        ge=0.0,
    )

    queue_processing_rate: int = Field(
        description="Rate at which queue is processed",
        ge=1,
    )
...
class ModelTicketRequirements(BaseModel):
    ...
    preferred_capacity: int | None = Field(
        default=None,
        description="Preferred lane capacity for execution",
        ge=1,
    )
    ...
    resource_requirements: dict[str, str] = Field(
        ...
        description="Resource requirements mapping",
    )

    execution_constraints: dict[str, bool] = Field(
        default_factory=dict,
        description="Execution constraints and flags",
    )
...
class ModelCriticalPathData(BaseModel):
    ...
    total_effort_hours: float = Field(
        description="Total effort hours for critical path",
        ge=0.0,
    )
    ...
    slack_analysis: dict[str, float] = Field(
        default_factory=dict,
        description="Slack time analysis for each node",
    )

    bottleneck_scores: dict[str, float] = Field(
        default_factory=dict,
        description="Bottleneck risk scores for each node",
    )
...
class ModelLeaseAnalysisCache(BaseModel):
    ...
    pressure_score: float = Field(
        description="Calculated TTL pressure score",
        ge=1.0,
        le=2.0,
    )
