...
class ModelFactorScore(BaseModel):
    ...
    raw_score: float = Field(
        ge=0.0, le=1.0, description="Raw calculated score before weighting (0.0-1.0)"
    )

    weight: float = Field(
        ge=0.0, le=1.0, description="Factor weight in final calculation"
    )

    weighted_score: float = Field(
        ge=0.0, le=1.0, description="Score after applying weight (raw_score * weight)"
    )

    components: List["ModelScoreComponent"] = Field(
        default_factory=list, description="Breakdown of how this factor was calculated"
    )
...
class ModelScoreComponent(BaseModel):
    ...
    normalized_value: float = Field(
        ge=0.0, le=1.0, description="Normalized value (0.0-1.0)"
    )
...
class ModelPriorityCalculation(BaseModel):
    ...
    final_score: float = Field(
        ge=0.0, le=1.0, description="Final calculated priority score (0.0-1.0)"
    )
    ...
    calculation_time: datetime = Field(
        default_factory=datetime.now, description="When this calculation was performed"
    )

    calculation_duration_ms: int = Field(
        ge=0, description="Time taken to calculate in milliseconds"
    )

    algorithm_version: str = Field(
        default="rsd-5-factor-v1", description="Algorithm version used"
    )

    override_applied: bool = Field(
        default=False, description="Whether a manual override affected the score"
    )

    override_details: Optional["ModelOverrideDetails"] = Field(
        default=None, description="Details of any override applied"
    )

    confidence_score: float = Field(
        ge=0.0, le=1.0, default=1.0, description="Overall confidence in the calculation"
    )

    warnings: List[str] = Field(
        default_factory=list, description="Any warnings during calculation"
    )
    ...
    def get_factor_by_name(self, name: str) -> Optional[ModelFactorScore]:
...
class ModelOverrideDetails(BaseModel):
    ...
    blend_strength: float = Field(
        ge=0.0, le=1.0, description="How much the override affected final score"
    )
...
class ModelBatchPriorityResult(BaseModel):
    ...
    successful_calculations: int = Field(
        ge=0, description="Number of successful calculations"
    )
    ...
    cache_hits: int = Field(
        ge=0, default=0, description="Number of calculations served from cache"
    )

    errors: List["ModelCalculationError"] = Field(
        default_factory=list, description="Any errors during batch calculation"
    )

    timestamp: datetime = Field(
        default_factory=datetime.now, description="When batch calculation was performed"
    )

    def get_sorted_tickets(self) -> List[str]:
        """Get ticket IDs sorted by priority score (highest first)."""
        sorted_calcs = sorted(
            self.calculations, key=lambda c: c.final_score, reverse=True
        )
        return [calc.ticket_id for calc in sorted_calcs]

    def get_calculation(self, ticket_id: str) -> Optional[ModelPriorityCalculation]:
