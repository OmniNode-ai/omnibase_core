...
class ModelPerformanceMetrics(BaseModel):
    ...
    average_storage_time_ms: float = Field(
        default=0.0,
        description="Average storage operation time in milliseconds",
    )
    average_query_time_ms: float = Field(
        default=0.0,
        description="Average query operation time in milliseconds",
    )
    average_embedding_time_ms: float = Field(
        default=0.0,
        description="Average embedding generation time in milliseconds",
    )
...
class ModelStatistics(BaseModel):
    ...
    total_conversations: int = Field(
        default=0,
        description="Total number of conversations stored",
    )
    ...
    total_chunks: int = Field(
        default=0,
        description="Total number of conversation chunks",
    )
    ...
    performance_metrics: ModelPerformanceMetrics | None = Field(
        default=None,
        description="Performance metrics for system operations",
    )

    # Usage analytics
    most_used_tools: list[ModelToolUsage] | None = Field(
        default=None,
        description="Most frequently used tools",
    )
    most_common_tags: list[ModelTagUsage] | None = Field(
        default=None,
    ...
    provider_name: str | None = Field(
        default=None,
        description="Vector store provider name",
    )
