...
class MixinEventHandler:
    ...
    def _handle_introspection_request(
        ...
        except Exception as e:  # noqa: BLE001  # catch-all-ok: introspection request handling errors are logged but shouldn't crash
            node_id = getattr(self, "_node_id", None) or "<unset>"
            context = ModelLogContext(
                calling_module=_COMPONENT_NAME,
                calling_function="_handle_introspection_request",
    ...
    def _handle_node_discovery_request(
        ...
        except Exception:  # noqa: BLE001  # fallback-ok: event handler returns early if type check fails, malformed events shouldn't crash
        ...
        except Exception as e:  # noqa: BLE001  # catch-all-ok: discovery request handling errors are logged but shouldn't crash
            node_id = getattr(self, "_node_id", None) or "<unset>"
            context = ModelLogContext(
                calling_module=_COMPONENT_NAME,
                calling_function="_handle_node_discovery_request",
    ...
    def _should_respond_to_request(self, event: OnexEvent) -> bool:
        ...
        except Exception:  # noqa: BLE001  # fallback-ok: filter error defaults to responding, safe fallback for event handling
