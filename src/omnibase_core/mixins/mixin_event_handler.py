...
class MixinEventHandler:
    ...
    def _handle_node_discovery_request(
        ...
        except Exception:  # fallback-ok: malformed event type check fails silently; malformed events must not crash handler
    ...
    def _should_respond_to_request(self, event: OnexEvent) -> bool:
        ...
        except Exception:  # fallback-ok: filter evaluation errors default to responding; safe for event handling
