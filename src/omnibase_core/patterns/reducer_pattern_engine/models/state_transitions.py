...
class WorkflowStateModel(BaseModel):
    ...
    def validate_state_consistency(cls, values):
        ...
        if current_state in [WorkflowState.PROCESSING, WorkflowState.COMPLETED, WorkflowState.FAILED]:
            if not started_at:
                values['started_at'] = values.get('created_at') or datetime.now()
        
        # If completed or failed, should have completed_at
        if current_state in [WorkflowState.COMPLETED, WorkflowState.FAILED, WorkflowState.CANCELLED]:
            if not completed_at:
    ...
    def transition_to(
        ...
        if new_state in [WorkflowState.COMPLETED, WorkflowState.FAILED, WorkflowState.CANCELLED]:
            ...
            if self.started_at:
                processing_duration = (self.completed_at - self.started_at).total_seconds() * 1000
    ...
    def can_transition_to(self, target_state: WorkflowState) -> bool:
        ...
        """
        return StateTransitionValidator.is_valid_transition(self.current_state, target_state)
