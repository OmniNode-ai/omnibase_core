...
class ModelWorkflowMetrics(BaseModel):
    ...
    total_workflows_processed: int = Field(
        0, description="Total number of workflows processed"
    )
    successful_workflows: int = Field(
        0, description="Number of successful workflow executions"
    )
    ...
    average_routing_time_ms: float = Field(
        0.0, description="Average routing time in milliseconds"
    )
    active_instances: int = Field(
        0, description="Number of currently active workflow instances"
    )
    subreducer_metrics: Dict[str, Dict[str, Any]] = Field(
        default_factory=dict, description="Per-subreducer performance metrics"
    )
