...
class ReducerReportGenerationSubreducer(BaseSubreducer):
    ...
    async def process(self, request: WorkflowRequest) -> SubreducerResult:
        ...
        try:
            ...
            # Process report data
            processed_data = self._process_report_data(report_config.get("data", {}), report_config)
            
            # Generate report content based on template
            template_type = report_config["template_type"]
            if template_type in self._template_processors:
                report_content = self._template_processors[template_type](processed_data, report_config)
            else:
                report_content = self._template_processors["custom"](processed_data, report_config)
            
            # Generate output in requested format
            output_format = report_config["output_format"]
            if output_format in self._supported_formats:
                formatted_output = self._supported_formats[output_format](report_content, report_config)
            else:
                # Default to JSON if format not supported
                formatted_output = self._supported_formats["json"](report_content, report_config)
                emit_log_event(
            ...
            report_metadata = self._generate_report_metadata(report_config, report_content, len(processed_data))
            
            # Validate generated report
            validation_results = self._validate_generated_report(formatted_output, report_config)
            
            # Calculate processing metrics
            processing_time = (time.time() - start_time) * 1000
            
            # Update metrics
            self._update_success_metrics(processing_time, template_type, output_format, len(report_content.get("sections", [])))
    ...
    def _validate_report_request(self, request: WorkflowRequest) -> None:
        ...
        if not request.payload:
            raise OnexError("Report generation request payload is required", CoreErrorCode.VALIDATION_FAILED)
        
        required_fields = ["template_type", "output_format"]
        for field in required_fields:
            if field not in request.payload:
                raise OnexError(f"{field} is required in report generation request", CoreErrorCode.VALIDATION_FAILED)
    ...
    def _process_report_data(self, data: Dict[str, Any], config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if template_config.get("include_aggregations", False):
            processed_data["_aggregations"] = self._compute_data_aggregations(processed_data)
    ...
    def _process_list_data(self, data: List[Any], key: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "unique_count": len(set(data)) if all(isinstance(x, (str, int, float)) for x in data) else None
        }
    ...
    def _process_numeric_data(self, data: Union[int, float], key: str) -> Dict[str, Any]:
    ...
    def _process_summary_template(self, data: Dict[str, Any], config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _process_detailed_template(self, data: Dict[str, Any], config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        sections.append({
            "title": "Introduction",
            "content": {
                "report_purpose": config.get("report_description", "Detailed analysis report"),
    ...
    def _process_dashboard_template(self, data: Dict[str, Any], config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _process_custom_template(self, data: Dict[str, Any], config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _generate_json_output(self, content: Dict[str, Any], config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _generate_html_output(self, content: Dict[str, Any], config: Dict[str, Any]) -> str:
    ...
    def _generate_csv_output(self, content: Dict[str, Any], config: Dict[str, Any]) -> str:
    ...
    def _generate_text_output(self, content: Dict[str, Any], config: Dict[str, Any]) -> str:
        """Generate plain text formatted output."""
        text_lines = [
            "=" * 50,
            content["title"].center(50),
            "=" * 50,
            ""
        ]
    ...
    def _generate_markdown_output(self, content: Dict[str, Any], config: Dict[str, Any]) -> str:
    ...
    def _generate_executive_summary(self, data: Dict[str, Any], config: Dict[str, Any]) -> Dict[str, Any]:
        """Generate executive summary from data."""
        return {
            "data_elements_analyzed": len([k for k in data.keys() if not k.startswith("_")]),
    ...
    def _generate_report_metadata(self, config: Dict[str, Any], content: Dict[str, Any], data_size: int) -> Dict[str, Any]:
    ...
    def _validate_generated_report(self, output: Any, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if config["output_format"] == "json":
            try:
                json.dumps(output)
                validation_results["validation_checks"].append("JSON format validation: PASSED")
            except Exception as e:
                validation_results["is_valid"] = False
                validation_results["errors"].append(f"JSON format validation failed: {str(e)}")
        
        return validation_results
    
    def _update_success_metrics(self, processing_time: float, template_type: str, output_format: str, sections_count: int) -> None:
        ...
        total_time = (self._processing_metrics["average_processing_time_ms"] * 
                     (self._processing_metrics["total_processed"] - 1) + processing_time)
        self._processing_metrics["average_processing_time_ms"] = total_time / self._processing_metrics["total_processed"]
    ...
    def _apply_custom_section_logic(self, data: Dict[str, Any], section_config: Dict[str, Any]) -> Any:
