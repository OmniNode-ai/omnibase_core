...
class ReducerDataAnalysisSubreducer(BaseSubreducer):
        ...
        try:
            ...
            for analysis_type in analysis_types:
                if analysis_type in self._supported_analyses:
                    ...
                    analysis_results[analysis_type] = analysis_func(cleaned_data, analysis_config)
                    
                    # Track analysis type usage
                    self._processing_metrics["analysis_types_used"][analysis_type] = (
                        self._processing_metrics["analysis_types_used"].get(analysis_type, 0) + 1
                    )
                else:
                    emit_log_event(
            ...
            analysis_summary = self._generate_analysis_summary(analysis_results, cleaned_data, analysis_config)
            ...
            return SubreducerResult(
                ...
                result={
                    ...
                    "data_statistics": {
                        ...
                        "data_quality_score": self._calculate_data_quality_score(data, cleaned_data)
                    }
                },
    ...
    def _validate_analysis_request(self, request: WorkflowRequest) -> None:
        ...
        if not request.payload:
            raise OnexError("Analysis request payload is required", CoreErrorCode.VALIDATION_FAILED)
        
        if "data" not in request.payload:
            raise OnexError("Data field is required in analysis request", CoreErrorCode.VALIDATION_FAILED)
        
        data = request.payload["data"]
        if not isinstance(data, list) or len(data) == 0:
            raise OnexError("Data must be a non-empty list", CoreErrorCode.VALIDATION_FAILED)
    ...
    def _extract_analysis_config(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        config = {
            ...
            "data_validation": payload.get("data_validation", {"remove_outliers": True, "handle_missing": True}),
            "statistical_config": payload.get("statistical_config", {"confidence_level": 0.95}),
            "output_format": payload.get("output_format", "detailed")
        }
        
        # Validate analysis types
        invalid_types = [t for t in config["analysis_types"] if t not in self._supported_analyses]
        if invalid_types:
            emit_log_event(
                level=LogLevel.WARNING,
            ...
            config["analysis_types"] = [t for t in config["analysis_types"] if t in self._supported_analyses]
    ...
    def _validate_and_clean_data(self, data: List[Any], config: Dict[str, Any]) -> List[float]:
        ...
        for item in data:
            try:
                ...
                if not validation_config.get("handle_missing", True):
                    raise OnexError(f"Invalid numeric value: {item}", CoreErrorCode.VALIDATION_FAILED)
                # Skip non-numeric values if configured to handle missing data
        
        if len(cleaned_data) == 0:
            raise OnexError("No valid numeric data points found", CoreErrorCode.VALIDATION_FAILED)
    ...
    def _perform_descriptive_analysis(self, data: List[float], config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _perform_correlation_analysis(self, data: List[float], config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if len(data) < 3:
            return {"autocorrelation_lag1": 0.0, "note": "Insufficient data for meaningful correlation"}
    ...
    def _perform_trend_analysis(self, data: List[float], config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "trend_strength": self._classify_trend_strength(abs(slope), max(data) - min(data)),
                "data_points": n
            }
    ...
    def _perform_distribution_analysis(self, data: List[float], config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _generate_analysis_summary(self, results: Dict[str, Any], data: List[float], config: Dict[str, Any]) -> Dict[str, Any]:
        """Generate comprehensive analysis summary."""
        summary = {
        ...
        if "descriptive" in results:
            ...
            if "mean" in desc and "std_dev" in desc:
                cv = desc["std_dev"] / desc["mean"] if desc["mean"] != 0 else float('inf')
                summary["key_insights"].append(f"Coefficient of variation: {cv:.3f} ({'high' if cv > 0.5 else 'moderate' if cv > 0.2 else 'low'} variability)")
        
        if "trend" in results:
            trend = results["trend"]
            if "trend_direction" in trend and "r_squared" in trend:
                summary["key_insights"].append(f"Trend: {trend['trend_direction']} with R² = {trend['r_squared']:.3f}")
    ...
    def _calculate_data_quality_score(self, original_data: List[Any], cleaned_data: List[float]) -> float:
        ...
        uniqueness = len(set(cleaned_data)) / len(cleaned_data) if len(cleaned_data) > 0 else 0
