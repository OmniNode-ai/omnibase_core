...
- Standalone / CI self-scan::
  ...
  OMNICLAUDE_ROOT defaults to ``$OMNIBASE_PATH/omniclaude`` when
  ``$OMNIBASE_PATH`` is set, else raises ``RuntimeError``.
...
def _resolve_omnimarket_nodes_root(omniclaude_root: Path) -> list[Path]:
    ...
    2. ``$OMNIBASE_PATH/omnimarket`` -- standard registry layout.
    ...
    bootstrap = ModelEnvironmentBootstrap.capture_process_environment(
        declared_keys=(_OMNIMARKET_ROOT_KEY, _OMNIBASE_PATH_KEY)
    )
    ...
    omnibase_path = bootstrap.environment.optional(_OMNIBASE_PATH_KEY)
    if omnibase_path:
        bases.append(Path(omnibase_path) / "omnimarket" / _OMNIMARKET_NODES_REL)
...
def scan(omniclaude_root: Path) -> list[str]:
    ...
    if not _omnimarket_available(omniclaude_root):
        print(
            ...
            "Set $OMNIMARKET_ROOT or $OMNIBASE_PATH to enable local enforcement. "
...
def _resolve_omniclaude_root() -> Path:
    ...
    omnibase_path = bootstrap.environment.optional(_OMNIBASE_PATH_KEY)
    if omnibase_path:
        candidate = Path(omnibase_path) / "omniclaude"
        if candidate.is_dir():
            return candidate
    raise RuntimeError(  # error-ok: CLI entrypoint environment misconfiguration — stdlib is appropriate
        ...
        "Pass the path as a CLI argument or set $OMNIBASE_PATH."
    )
