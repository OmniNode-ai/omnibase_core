...
"""Two-transport runner for the private-IP COMPUTE validator (OMN-13294, §1A).

The seam that makes the SAME ``HandlerPrivateIpCompute`` run over a swappable bus
backend (identical to the G1 local-paths canary):
...
class PrivateIpBusRunner:
    ...
    def __init__(self, bus: EventBusInmemory) -> None:
        self._bus = bus
        self._handler = HandlerPrivateIpCompute()
        self._results: dict[str, ModelPrivateIpScanResult] = {}

    async def _on_command(self, message: ModelEventMessage) -> None:
        raw = json.loads(message.value.decode("utf-8"))
        scan_input = ModelPrivateIpScanInput.model_validate(raw)
    ...
    async def _on_result(self, message: ModelEventMessage) -> None:
        raw = json.loads(message.value.decode("utf-8"))
        result = ModelPrivateIpScanResult.model_validate(raw)
        self._results[result.path] = result

    async def scan_inputs(
        self, inputs: list[ModelPrivateIpScanInput]
    ) -> list[ModelPrivateIpScanResult]:
...
async def _run(paths: list[Path], *, quiet: bool) -> int:
    inputs = [
        ModelPrivateIpScanInput(content=_read_text(fp), path=str(fp))
    ...
    for result in flagged:
        for finding in result.findings:
            ...
            print(
                ...
                f"[{finding.rfc1918_block}] {finding.matched_text!r}"
            )
            if not quiet:
    ...
    if not quiet:
        if total_findings:
            print(
                f"\n{total_findings} hardcoded private-IP violation(s). Resolve the "
                f"endpoint from the routing authority / contract, or add "
                f"`# {'onex-allow-internal-ip'}` to suppress an approved fixture."
            )
        else:
            print("No hardcoded private-IP violations found.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        ...
        description=(
            "Detect hardcoded RFC1918 private-IP literals via the private-IP "
            "COMPUTE node executed over the in-memory event bus (OMN-13294)."
        ),
    )
    parser.add_argument(
        "paths",
