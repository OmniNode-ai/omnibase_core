...
"""Routing-authority check logic (OMN-13306, W6b; OMN-3210 layering burn-down).

Pure computation for the routing-authority verification gate, plus the
filesystem-reading convenience helper used by the pre-commit CLI wrapper
(``validator_routing_authority.py``).

This module is the canonical home for the check algorithm. It used to live
inside ``nodes/node_routing_authority_check_compute/handler.py``, but a
``validation/`` module calling into node-internal implementation created a
frozen `validation -> nodes` import-layering back-edge (see
``docs/architecture/DEPENDENCY_INVERSION.md`` / the `.importlinter`
`core-midtier-no-top` contract). ``NodeRoutingAuthorityCheckCompute`` now
delegates to the functions here (nodes -> validation is a doctrine-legal
direction); this module has no dependency on ``omnibase_core.nodes``.
...
The `run_routing_authority_check()` entry point is pure, deterministic, no I/O.
`check_routing_authority_at_path()` is the EFFECT boundary — it reads files
from the filesystem and delegates to `run_routing_authority_check()`.
...
def _build_positive_proof(
    ...
    for entry in inp.demo_path_contracts:
        ...
        if not contract_text:
            errors.append(f"demo-path contract content missing: {entry.contract_rel}")
            continue
        ...
        if not isinstance(data, dict):
            errors.append(f"{entry.contract_rel}: contract did not parse to a mapping")
            continue
        ...
        if isinstance(endpoint_ref, str) and endpoint_ref:
            endpoint_url, _api_key_ref, endpoint_source = resolve_endpoint_for_ref(
                endpoint_ref,
...
# ---------------------------------------------------------------------------
# Top-level check entry point (pure — mirrors NodeRoutingAuthorityCheckCompute.check)
# ---------------------------------------------------------------------------


def run_routing_authority_check(
    inp: ModelRoutingAuthorityCheckInput,
) -> ModelRoutingAuthorityCheckOutput:
    ...
    """
    positive = _build_positive_proof(inp)
    negative = _build_negative_audit(inp)
    residue = _build_residue_audit(inp)
    shape = _build_provider_endpoint_shape_audit(inp)
...
def check_routing_authority_at_path(
    ...
    ``run_routing_authority_check()``. Called by the pre-commit CLI wrapper.
    """
