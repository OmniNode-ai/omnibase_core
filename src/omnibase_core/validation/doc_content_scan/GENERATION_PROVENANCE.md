<!-- doc-content-file-ok OMN-13569 reason="this provenance file's subject IS the local-env traces / endpoint the doc-content validator scans for; the literals + the OMN ids are intentional evidence" -->
<!-- onex-allow-file-internal-ip OMN-13569 reason="this provenance documents the live generation backend endpoint; the IP literal is evidence, not config" -->

# Doc-Content COMPUTE Validator — Generation Provenance (OMN-13569, G2)
...
| resolved_endpoint | `http://192.168.86.201:8000/v1/chat/completions` (local-coder backend) | <!-- onex-allow-internal-ip OMN-13569 generation-evidence endpoint -->
...
against a fixture corpus of **15 violation fixtures (9 adversarial mutation cases)
...
knowledge-base sanitizer (strip OMN ids / IPs / `.201` / private URLs / e-mail) —
...
the `OMNI_HOME` / `OMNINODE` tokens that are NOT the `OMN-<digits>` ticket shape.

The corpus is a TEXT-only acceptance authority (the sandbox only ever sees
`input_data["source"]`, never a path). The **path-based** `OMN-<digits>` exemptions
...
tokens (host shorthand), and bounded to `OMN-<digits>` standalone tokens (so
...
real docs tree) is tracked separately as **OMN-13572** — deliberately deferred so
no untested validator is blanket-blocked. Nothing in `.pre-commit-hooks.yaml`,
