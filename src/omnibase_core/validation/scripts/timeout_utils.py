...
class CrossPlatformTimeout:
    ...
    def __exit__(self, exc_type: Any, exc_val: Any, exc_tb: Any) -> None:
        ...
        if self.timed_out:
            # error-ok: stdlib TimeoutError for cross-platform timeout mechanism in validation script
...
class UnixSignalTimeout:
    ...
    def _signal_handler(self, signum: int, frame: Any) -> None:
        ...
        # error-ok: stdlib TimeoutError required in SIGALRM handler — OnexError not safe in signal context
        raise TimeoutError(self.error_message)
...
def safe_file_operation(  # noqa: UP047
    ...
    with CrossPlatformTimeout(timeout_seconds, error_message):
        try:
            ...
            # error-ok: stdlib OSError for file I/O in validation script — OnexError not appropriate here
