...
class ModelContractCompute(ModelContractBase):
    ...
    def validate_node_specific_config(self, original_contract_data: Optional[Dict] = None) -> None:
        ...
        if self.caching and hasattr(self.caching, 'max_entries') and self.caching.max_entries < 1:
    ...
    def validate_subcontract_constraints(self, original_contract_data: Optional[Dict] = None) -> None:
        ...
        if "state_management" in contract_data:
            violations.append("❌ SUBCONTRACT VIOLATION: COMPUTE nodes cannot have state_management subcontracts")
        ...
        if "aggregation" in contract_data:
            violations.append("❌ SUBCONTRACT VIOLATION: COMPUTE nodes cannot have aggregation subcontracts")
        ...
        if "state_transitions" in contract_data:
            violations.append("❌ SUBCONTRACT VIOLATION: COMPUTE nodes cannot have state_transitions subcontracts")
