...
from omnibase.model.core.model_connection_metrics import ModelConnectionMetrics
...
class ModelCustomConnectionProperties(BaseModel):
    ...
    durable: Optional[bool] = Field(None, description="Durable queue/exchange")
    ...
    connection_limit: Optional[int] = Field(None, description="Connection limit")
    command_timeout: Optional[int] = Field(None, description="Command timeout")

    # Compression and optimization
    enable_compression: Optional[bool] = Field(None, description="Enable compression")
    ...
    custom_strings: Optional[Dict[str, str]] = Field(
        None, description="Additional string properties"
    )
    # Custom numeric properties
    custom_numbers: Optional[Dict[str, float]] = Field(
        None, description="Additional numeric properties"
    )
    # Custom boolean flags
    custom_flags: Optional[Dict[str, bool]] = Field(
        None, description="Additional boolean flags"
    )
...
class ModelConnectionInfo(BaseModel):
    ...
    path: Optional[str] = Field(None, description="Connection path/endpoint")
    ...
    token: Optional[SecretStr] = Field(None, description="Auth token (encrypted)")
    ...
    ssl_ca_path: Optional[str] = Field(None, description="SSL CA bundle path")
    ...
    keepalive_interval: Optional[int] = Field(
        None, description="Keepalive interval in seconds"
    )
    ...
    max_overflow: Optional[int] = Field(None, description="Maximum pool overflow")
    ...
    connection_state: str = Field(
        "disconnected", description="Current connection state"
    )

    # Metrics
    metrics: Optional[ModelConnectionMetrics] = Field(
        None, description="Connection metrics"
    )
