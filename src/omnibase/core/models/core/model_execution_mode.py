...
class ModelExecutionMode(BaseModel):
    ...
    instead of methods while maintaining backward compatibility.
    """

    # Core fields (required)
    name: str = Field(
        ...,
        description="Execution mode identifier (DIRECT, INMEMORY, KAFKA)",
        pattern="^[A-Z]+$",
    )
    ...
    @classmethod
    def KAFKA(cls) -> "ModelExecutionMode":
        """Kafka execution mode - asynchronous, distributed event bus."""
        return cls(
            name="KAFKA",
            value="kafka",
            is_synchronous=False,
            ...
            description="Distributed asynchronous execution with Kafka",
    ...
    def from_string(cls, mode: str) -> "ModelExecutionMode":
        """Create ModelExecutionMode from string for backward compatibility."""
    ...
    # Backward compatibility methods (same signatures as enum)
