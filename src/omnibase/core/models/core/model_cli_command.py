...
from omnibase.model.core.model_event_type import ModelEventType
...
class ModelCLICommand(BaseModel):
    ...
    required_args: List[ModelCLIArgument] = Field(
        default_factory=list, description="Required arguments for this command"
    )
    optional_args: List[ModelCLIArgument] = Field(
        default_factory=list, description="Optional arguments for this command"
    )
    examples: List[str] = Field(
        default_factory=list, description="Usage examples for this command"
    )
