...
class TestMultiWorkflowProcessing:
    ...
    async def test_concurrent_workflow_processing(self, engine, sample_workflows):
        ...
        tasks = [
            engine.process_workflow(request) for request in sample_workflows.values()
        ]
        ...
        ), f"Concurrent processing ({total_time:.3f}s) should be faster than sequential ({sequential_time:.3f}s)"
    ...
    async def test_bulk_subreducer_registration(self, engine, sample_workflows):
        ...
        expected_types = {
            request.workflow_type.value for request in sample_workflows.values()
        }
