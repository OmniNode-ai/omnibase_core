...
class MockMultiTypeSubreducer(BaseSubreducer):
    ...
    def __init__(self, name: str = "mock_multi_subreducer"):
        super().__init__(name)
        self._supported_types = [WorkflowType.DATA_ANALYSIS, WorkflowType.REPORT_GENERATION]
...
class TestReducerSubreducerRegistry:
    ...
    def test_overwrite_existing_registration(self, registry):
        ...
        with patch('omnibase_core.patterns.reducer_pattern_engine.v1_0_0.registry.emit_log_event'):
    ...
    def test_get_registry_summary(self, registry):
        ...
        subreducers_config = [
            ...
            (WorkflowType.REPORT_GENERATION, MockMultiTypeSubreducer, {"version": "1.1"}),
        ]
        ...
        assert reg_metadata[WorkflowType.DATA_ANALYSIS.value]["metadata"]["version"] == "1.0"
        assert reg_metadata[WorkflowType.REPORT_GENERATION.value]["metadata"]["version"] == "1.1"
    ...
    def test_clear_registry(self, registry):
        ...
        registry.register_subreducer(WorkflowType.REPORT_GENERATION, MockMultiTypeSubreducer)
    ...
    def test_metadata_immutability(self, registry):
        ...
        summary = registry.get_registry_summary()
        summary["registration_metadata"][workflow_type.value]["metadata"]["another_modification"] = "test"
        
        fresh_summary = registry.get_registry_summary()
        assert "another_modification" not in fresh_summary["registration_metadata"][workflow_type.value]["metadata"]