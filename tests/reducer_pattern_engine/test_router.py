...
def router():
    ...
    with patch('omnibase_core.patterns.reducer_pattern_engine.v1_0_0.router.emit_log_event'):
...
class TestSubreducerRegistration:
    ...
    def test_register_subreducer_success(self, router):
        ...
        assert router._workflow_type_mappings[WorkflowType.DOCUMENT_REGENERATION] == "doc_regeneration_subreducer"
...
class TestWorkflowRouting:
    ...
    async def test_successful_routing(self, router, sample_workflow_request):
        ...
        assert decision.routing_metadata["correlation_id"] == str(sample_workflow_request.correlation_id)
        assert decision.routing_metadata["routing_algorithm"] == "hash_based"
        assert decision.routing_metadata["total_subreducers"] == 1
    
    @pytest.mark.asyncio
    async def test_routing_unsupported_workflow_type(self, router, sample_workflow_request):
    ...
    async def test_routing_error_updates_metrics(self, router, sample_workflow_request):
        ...
        # Check error metrics
        updated_metrics = router.get_routing_metrics()
        assert updated_metrics["routing_errors"] == initial_metrics["routing_errors"] + 1
...
class TestRoutingMetrics:
    ...
    async def test_routing_metrics_average_calculation(self, router):
        ...
        assert metrics["average_routing_time_ms"] < 100.0  # Should be much less than 100ms
...
class TestErrorHandling:
    ...
    @pytest.mark.asyncio
    async def test_routing_with_exception_during_processing(self, router, sample_workflow_request):
        ...
        with patch('omnibase_core.patterns.reducer_pattern_engine.v1_0_0.router.emit_log_event'):
            ...
            with patch.object(router, '_generate_routing_hash', side_effect=Exception("Hash generation failed")):
    ...
    def test_registration_error_logging(self, router):
        ...
        with patch('omnibase_core.patterns.reducer_pattern_engine.v1_0_0.router.emit_log_event') as mock_log:
            ...
            error_calls = [call for call in mock_log.call_args_list if 'registration_failed' in str(call)]
