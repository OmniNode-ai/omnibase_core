...
class TestReducerReportGenerationSubreducer:
    ...
    def test_supports_workflow_type(self, subreducer):
        ...
        assert subreducer.supports_workflow_type(WorkflowType.DOCUMENT_REGENERATION) is False
    ...
    async def test_process_successful_summary_report_json(self, subreducer):
        ...
        assert len(report_content["sections"]) >= 2  # At least executive summary and data overview
    ...
    async def test_process_complex_data_structures(self, subreducer):
        ...
        gen_stats = result.result["generation_statistics"]
        assert gen_stats["data_points_processed"] == 4  # nested_dict, mixed_list, numeric_data, text_data
        assert gen_stats["sections_generated"] >= 4  # Introduction + data sections + conclusions
    ...
    def test_format_validation(self, subreducer):
        ...
        assert any("JSON format validation: PASSED" in check for check in validation_results["validation_checks"])
    ...
    async def test_error_handling_during_processing(self, subreducer):
        ...
        with patch.object(subreducer, '_process_report_data', side_effect=ValueError("Mock processing error")):
            request = WorkflowRequest(
                workflow_id=uuid4(),
                workflow_type=WorkflowType.REPORT_GENERATION,
            ...
            assert result.error_details["failed_at_step"] == "report_generation_processing"
    ...
    async def test_concurrent_processing_safety(self, subreducer):
        ...
        for i in range(10):
            request = WorkflowRequest(
                ...
                payload={
                    ...
                    "data": {f"data_{i}": list(range(i + 1, i + 6))}  # Different data for each
                }
            )
        ...
        report_titles = [
            result.result["report_content"]["report"]["title"] 
    ...
    async def test_large_data_performance(self, subreducer):
        ...
        assert gen_stats["sections_generated"] >= 50  # At least one section per dataset + intro/conclusion
    ...
    async def test_all_output_formats_consistency(self, subreducer):
        ...
        for output_format, result in results.items():
            ...
            assert result.result["generation_statistics"]["output_format"] == output_format
