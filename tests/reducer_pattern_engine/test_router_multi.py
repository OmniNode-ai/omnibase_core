...
class MockRouterSubreducer(BaseSubreducer):
    ...
    async def process(self, request: WorkflowRequest) -> SubreducerResult:
        ...
        return SubreducerResult(
            ...
            success=True,
            result={"routed_successfully": True, "call_number": self._process_call_count}
        )
...
class TestWorkflowRouterEnhanced:
    ...
    def test_register_single_workflow_type_subreducer(self, router, mock_subreducers):
        ...
        assert router._workflow_type_mappings[WorkflowType.DATA_ANALYSIS] == subreducer.name
        
        # Test retrieval
        retrieved = router.get_subreducer(subreducer.name)
        assert retrieved == subreducer
    
    def test_register_multiple_workflow_type_subreducers(self, router, mock_subreducers):
        ...
        subreducers_to_register = [
            ...
            (mock_subreducers["document_regeneration"], [WorkflowType.DOCUMENT_REGENERATION])
        ]
        ...
        assert router._workflow_type_mappings[WorkflowType.DATA_ANALYSIS] == "data_analysis_subreducer"
        assert router._workflow_type_mappings[WorkflowType.REPORT_GENERATION] == "report_generation_subreducer"
        assert router._workflow_type_mappings[WorkflowType.DOCUMENT_REGENERATION] == "document_regeneration_subreducer"
    ...
    def test_register_multi_type_subreducer(self, router, mock_subreducers):
        ...
        assert router._workflow_type_mappings[WorkflowType.DATA_ANALYSIS] == multi_subreducer.name
        assert router._workflow_type_mappings[WorkflowType.REPORT_GENERATION] == multi_subreducer.name
    ...
    def test_register_duplicate_workflow_type(self, router, mock_subreducers):
        ...
        assert len(router._subreducers) == 1
        assert router._workflow_type_mappings[WorkflowType.DATA_ANALYSIS] == first_subreducer.name
    
    @pytest.mark.asyncio
    async def test_route_single_workflow_type(self, router, mock_subreducers, sample_requests):
    ...
    @pytest.mark.asyncio
    async def test_route_multiple_workflow_types(self, router, mock_subreducers, sample_requests):
        ...
        assert routing_results["data_analysis"].subreducer_name == "data_analysis_subreducer"
        assert routing_results["report_generation"].subreducer_name == "report_generation_subreducer"
        assert routing_results["document_regeneration"].subreducer_name == "document_regeneration_subreducer"
    ...
    async def test_route_unsupported_workflow_type(self, router, mock_subreducers):
        ...
        router.register_subreducer(mock_subreducers["data_analysis"], [WorkflowType.DATA_ANALYSIS])
    ...
    @pytest.mark.asyncio
    async def test_routing_metrics_tracking(self, router, mock_subreducers, sample_requests):
        ...
        router.register_subreducer(mock_subreducers["report_generation"], [WorkflowType.REPORT_GENERATION])
    ...
    async def test_concurrent_routing_performance(self, router, mock_subreducers):
        ...
        concurrent_requests = []
        for i in range(100):
            workflow_type = [WorkflowType.DATA_ANALYSIS, WorkflowType.REPORT_GENERATION, WorkflowType.DOCUMENT_REGENERATION][i % 3]
            request = WorkflowRequest(
                workflow_id=uuid4(),
        ...
        for decision in decisions:
            subreducer_counts[decision.subreducer_name] = subreducer_counts.get(decision.subreducer_name, 0) + 1
        ...
        print(f"Routed {len(concurrent_requests)} requests concurrently in {total_time:.3f}s")
        print(f"Average routing time: {metrics['average_routing_time_ms']:.3f}ms")
    
    @pytest.mark.asyncio
    async def test_routing_decision_structure(self, router, mock_subreducers, sample_requests):
    ...
    async def test_error_handling_and_logging(self, router, mock_subreducers):
        ...
        with patch('omnibase_core.patterns.reducer_pattern_engine.v1_0_0.router.emit_log_event') as mock_log:
            # Successful routing
            request = WorkflowRequest(
                workflow_id=uuid4(),
                workflow_type=WorkflowType.DATA_ANALYSIS,
            ...
            success_calls = [call for call in mock_log.call_args_list if "workflow_routed" in str(call)]
            ...
            error_calls = [call for call in mock_log.call_args_list if "routing_failed" in str(call)]
