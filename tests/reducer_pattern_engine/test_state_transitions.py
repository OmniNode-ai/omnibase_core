...
class TestStateTransition:
    """Test StateTransition model functionality."""
    
    def test_state_transition_creation(self):
        """Test basic StateTransition model creation."""
        ...
        assert transition.reason == "Started processing"
    ...
    def test_state_transition_json_serialization(self):
        """Test StateTransition JSON serialization."""
        transition = StateTransition(
            from_state=WorkflowState.PENDING,
            to_state=WorkflowState.PROCESSING,
        ...
        assert transition_dict["reason"] == "Test transition"
        assert transition_dict["metadata"] == {"test": True}
        
        # Should be able to recreate from dict
        recreated = StateTransition(**transition_dict)
...
class TestWorkflowStateModel:
    """Test WorkflowStateModel comprehensive functionality."""
    
    def test_workflow_state_model_creation(self):
        """Test basic WorkflowStateModel creation."""
        workflow_id = uuid4()
        ...
        assert state_model.workflow_id == workflow_id
        assert state_model.workflow_type == "data_analysis"
        assert state_model.correlation_id == correlation_id
        assert state_model.instance_id == "test-instance-001"
        assert state_model.metadata == {"test": "value"}
        assert state_model.current_state == WorkflowState.PENDING
        assert state_model.previous_state is None
        assert state_model.retry_count == 0
        assert state_model.max_retries == 3
        assert isinstance(state_model.created_at, datetime)
        assert isinstance(state_model.updated_at, datetime)
        assert state_model.transition_history == []
    
    def test_workflow_state_model_validation(self):
        """Test WorkflowStateModel validation rules."""
        workflow_id = uuid4()
        correlation_id = uuid4()
        
        # Test workflow_type validation
        with pytest.raises(ValueError, match="Workflow type must be a non-empty string"):
            WorkflowStateModel(
                workflow_id=workflow_id,
                workflow_type="",
                correlation_id=correlation_id
    ...
    def test_transition_to_retrying(self):
        ...
        state_model.transition_to(WorkflowState.RETRYING, reason="Retrying after failure")
...
class TestStateTransitionValidator:
    ...
    def test_get_valid_transitions(self):
        ...
        assert pending_transitions == {WorkflowState.PROCESSING, WorkflowState.CANCELLED}
        
        # PROCESSING state transitions
        processing_transitions = validator.get_valid_transitions(WorkflowState.PROCESSING)
    ...
    def test_complex_workflow_scenarios(self):
        ...
        state_model.transition_to(WorkflowState.COMPLETED, reason="Work finished successfully")
