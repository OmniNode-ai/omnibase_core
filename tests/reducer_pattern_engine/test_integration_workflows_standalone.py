...
class WorkflowRequest(BaseModel):
    ...
    instance_id: str = Field(..., description="Unique instance identifier for isolation")
...
class MockSubreducer:
    ...
    def __init__(self, name: str, supported_type: WorkflowType, success_rate: float = 0.9):
        self.name = name
    ...
    def supports_workflow_type(self, workflow_type: WorkflowType) -> bool:
        ...
        actual_type = workflow_type.value if hasattr(workflow_type, 'value') else workflow_type
        expected_type = self.supported_type.value if hasattr(self.supported_type, 'value') else self.supported_type
...
class MockRouter:
    ...
    def route_workflow(self, request: WorkflowRequest, subreducers: List[MockSubreducer]) -> Optional[MockSubreducer]:
...
class MockIntegrationEngine:
    ...
    def __init__(self):
        ...
        self.subreducers = [
            MockSubreducer("data_analysis_subreducer", WorkflowType.DATA_ANALYSIS, 0.95),
            MockSubreducer("document_regen_subreducer", WorkflowType.DOCUMENT_REGENERATION, 0.90),
            MockSubreducer("report_gen_subreducer", WorkflowType.REPORT_GENERATION, 0.85),
        ]
        ...
        try:
            ...
            if subreducer is None:
                ...
                response = WorkflowResponse(
                    ...
                    error_details={"workflow_type": request.workflow_type.value if hasattr(request.workflow_type, 'value') else request.workflow_type},
                )
...
class TestStandaloneIntegrationWorkflows:
    ...
    async def test_complete_data_analysis_workflow_end_to_end(self, integration_engine):
        ...
        # Handle both enum and string values due to Pydantic config
        expected_type = WorkflowType.DATA_ANALYSIS.value if hasattr(WorkflowType.DATA_ANALYSIS, 'value') else WorkflowType.DATA_ANALYSIS
        actual_type = response.workflow_type.value if hasattr(response.workflow_type, 'value') else response.workflow_type
        assert actual_type == expected_type
        assert response.instance_id == request.instance_id
        assert response.correlation_id == request.correlation_id
        # Handle both enum and string values for status
        actual_status = response.status.value if hasattr(response.status, 'value') else response.status
        assert actual_status in [WorkflowStatus.COMPLETED.value, WorkflowStatus.FAILED.value]
        # Only check processing time if not None (failures might not have it)
        if response.processing_time_ms is not None:
            assert response.processing_time_ms > 0
        
        # Handle enum/string comparison for status
        response_status = response.status.value if hasattr(response.status, 'value') else response.status
    ...
    @pytest.mark.asyncio
    async def test_complete_document_regeneration_workflow_end_to_end(self, integration_engine):
    ...
    @pytest.mark.asyncio
    async def test_complete_report_generation_workflow_end_to_end(self, integration_engine):
    ...
    async def test_error_recovery_integration_mixed_outcomes(self, integration_engine):
        ...
        for response in responses:
            ...
            response_status = response.status.value if hasattr(response.status, 'value') else response.status
            state_status = state.current_state.value if hasattr(state.current_state, 'value') else state.current_state
        ...
        assert metrics["successful_workflows"] + metrics["failed_workflows"] <= len(responses)
    ...
    @pytest.mark.asyncio
    async def test_state_management_throughout_workflow_lifecycle(self, integration_engine):
    ...
    async def test_performance_metrics_integration(self, integration_engine):
        ...
        for workflow_type in WorkflowType:
            ...
            type_times = [r.processing_time_ms for r in type_responses if r.processing_time_ms]
            if type_times:
    ...
    async def test_comprehensive_system_integration(self, integration_engine):
        ...
        assert metrics["successful_workflows"] + metrics["failed_workflows"] == len(test_scenarios)
    ...
    async def test_full_system_stress_integration(self, integration_engine):
        ...
        for batch_num in range(num_stress_workflows // batch_size):
            ...
            for i in range(batch_size):
                ...
                workflow_type = [WorkflowType.DATA_ANALYSIS, WorkflowType.DOCUMENT_REGENERATION, WorkflowType.REPORT_GENERATION][workflow_idx % 3]
