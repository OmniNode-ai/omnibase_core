...
"""Unit tests for ModelHandlerArtifactRef."""
...
@pytest.mark.unit
class TestModelHandlerArtifactRefInstantiation:
    """Tests for ModelHandlerArtifactRef instantiation."""
...
@pytest.mark.unit
class TestModelHandlerArtifactRefOptionalFields:
    """Tests for ModelHandlerArtifactRef optional fields."""

    def test_optional_digest_default_none(self) -> None:
        """Test digest defaults to None."""
        ref = ModelHandlerArtifactRef(ref="artifact://test")
        assert ref.digest is None
    ...
    def test_version_can_be_explicitly_none(self) -> None:
        ...
        ref = ModelHandlerArtifactRef(ref="artifact://test", version=None)
        assert ref.version is None


@pytest.mark.unit
class TestModelHandlerArtifactRefImmutability:
    """Tests for ModelHandlerArtifactRef frozen immutability."""
    ...
    def test_frozen_immutability_digest(self) -> None:
        ...
        ref = ModelHandlerArtifactRef(ref="artifact://test", digest="sha256:abc")
        with pytest.raises(ValidationError, match="frozen"):
    ...
    def test_frozen_immutability_version(self) -> None:
        ...
        version = ModelSemVer(major=1, minor=0, patch=0)
        ref = ModelHandlerArtifactRef(ref="artifact://test", version=version)
    ...
    def test_frozen_immutability_source(self) -> None:
        ...
        ref = ModelHandlerArtifactRef(
            ref="artifact://test", source="https://example.com"
        )
        with pytest.raises(ValidationError, match="frozen"):
...
@pytest.mark.unit
class TestModelHandlerArtifactRefExtraFields:
...
@pytest.mark.unit
class TestModelHandlerArtifactRefValidation:
    """Tests for ModelHandlerArtifactRef validation."""

    def test_empty_ref_rejected(self) -> None:
        """Test that empty ref is rejected (min_length=1)."""
        with pytest.raises(ValidationError) as exc_info:
            ModelHandlerArtifactRef(ref="")
    ...
    def test_whitespace_only_ref_accepted(self) -> None:
        ...
        ref = ModelHandlerArtifactRef(ref=" ")
        assert ref.ref == " "

    def test_various_ref_formats_accepted(self) -> None:
        """Test that various ref formats are accepted (opaque identifier)."""
        # URI format
        ref1 = ModelHandlerArtifactRef(ref="artifact://path/to/resource")
        assert ref1.ref == "artifact://path/to/resource"

        # URN format
        ref2 = ModelHandlerArtifactRef(ref="urn:artifact:v1:abc123")
        assert ref2.ref == "urn:artifact:v1:abc123"

        # Path format
        ref3 = ModelHandlerArtifactRef(ref="/path/to/artifact.yaml")
        assert ref3.ref == "/path/to/artifact.yaml"

        # Custom slug format
        ref4 = ModelHandlerArtifactRef(ref="my-artifact-v1")
        assert ref4.ref == "my-artifact-v1"


@pytest.mark.unit
class TestModelHandlerArtifactRefSerialization:
    """Tests for ModelHandlerArtifactRef serialization."""
    ...
    def test_model_dump_roundtrip(self) -> None:
        ...
        version = ModelSemVer(major=1, minor=0, patch=0)
        original = ModelHandlerArtifactRef(
        ...
        restored = ModelHandlerArtifactRef(**data)
