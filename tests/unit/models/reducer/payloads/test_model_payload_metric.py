...
"""
Tests for PayloadMetric.

This module tests the PayloadMetric model for metric recording intents, verifying:
...
@pytest.mark.unit
class TestPayloadMetricInstantiation:
    """Test PayloadMetric instantiation."""

    def test_create_with_required_fields(self) -> None:
        """Test creating payload with required fields only."""
        payload = PayloadMetric(name="test.metric", value=42.0)
...
@pytest.mark.unit
class TestPayloadMetricNameValidation:
    """Test name field validation."""

    def test_name_required(self) -> None:
        """Test that name is required."""
        with pytest.raises(ValidationError) as exc_info:
            PayloadMetric(value=1.0)  # type: ignore[call-arg]
        assert "name" in str(exc_info.value)

    def test_name_min_length(self) -> None:
        """Test name minimum length validation."""
        with pytest.raises(ValidationError) as exc_info:
            PayloadMetric(name="", value=1.0)
        assert "name" in str(exc_info.value)
    ...
    def test_name_max_length(self) -> None:
        ...
        long_name = "a" * 257
        with pytest.raises(ValidationError) as exc_info:
            PayloadMetric(name=long_name, value=1.0)
        assert "name" in str(exc_info.value)
...
class TestPayloadMetricMetricTypeValidation:
    ...
    def test_invalid_metric_type_rejected(self) -> None:
        ...
        with pytest.raises(ValidationError) as exc_info:
            PayloadMetric(name="test", value=1.0, metric_type="invalid")  # type: ignore[arg-type]
        assert "metric_type" in str(exc_info.value)


@pytest.mark.unit
class TestPayloadMetricDefaultValues:
    """Test default values."""

    def test_default_metric_type(self) -> None:
        """Test default metric_type is gauge."""
        payload = PayloadMetric(name="test", value=1.0)
        assert payload.metric_type == "gauge"
...
class TestPayloadMetricUnitValidation:
    ...
    def test_unit_max_length(self) -> None:
        ...
        long_unit = "a" * 33
        with pytest.raises(ValidationError) as exc_info:
            PayloadMetric(name="test", value=1.0, unit=long_unit)
    ...
    def test_valid_unit_values(self) -> None:
        ...
        for unit in valid_units:
            payload = PayloadMetric(name="test", value=1.0, unit=unit)
            assert payload.unit == unit


@pytest.mark.unit
class TestPayloadMetricImmutability:
...
@pytest.mark.unit
class TestPayloadMetricSerialization:
    """Test serialization/deserialization."""

    def test_roundtrip_serialization(self) -> None:
        """Test roundtrip serialization."""
        original = PayloadMetric(
        ...
        restored = PayloadMetric.model_validate(data)
        assert restored == original

    def test_json_roundtrip(self) -> None:
        """Test JSON roundtrip serialization."""
        original = PayloadMetric(name="test", value=42.0)
        json_str = original.model_dump_json()
        restored = PayloadMetric.model_validate_json(json_str)
        assert restored == original
...
@pytest.mark.unit
class TestPayloadMetricExtraFieldsRejected:
