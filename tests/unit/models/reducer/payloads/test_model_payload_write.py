...
"""
Tests for PayloadWrite.

This module tests the PayloadWrite model for file/storage write intents, verifying:
...
@pytest.mark.unit
class TestPayloadWriteInstantiation:
    """Test PayloadWrite instantiation."""

    def test_create_with_required_fields(self) -> None:
        """Test creating payload with required fields only."""
        payload = PayloadWrite(path="/test/path", content="test content")
...
@pytest.mark.unit
class TestPayloadWriteDefaultValues:
    """Test default values."""
...
class TestPayloadWriteValidation:
    ...
    def test_path_max_length(self) -> None:
        ...
        long_path = "a" * 1025
        with pytest.raises(ValidationError) as exc_info:
            PayloadWrite(path=long_path, content="data")
        assert "path" in str(exc_info.value)
    ...
    def test_content_type_max_length(self) -> None:
        ...
        with pytest.raises(ValidationError) as exc_info:
            PayloadWrite(path="/test", content="data", content_type=long_content_type)
    ...
    def test_encoding_max_length(self) -> None:
        ...
        long_encoding = "a" * 33
        with pytest.raises(ValidationError) as exc_info:
            PayloadWrite(path="/test", content="data", encoding=long_encoding)
        assert "encoding" in str(exc_info.value)


@pytest.mark.unit
class TestPayloadWriteImmutability:
...
@pytest.mark.unit
class TestPayloadWriteSerialization:
    """Test serialization/deserialization."""

    def test_roundtrip_serialization(self) -> None:
        """Test roundtrip serialization."""
        original = PayloadWrite(
            path="/test/path",
            content="test content",
            content_type="application/json",
            metadata={"key": "value"},
        ...
        restored = PayloadWrite.model_validate(data)
        assert restored == original

    def test_json_roundtrip(self) -> None:
        """Test JSON roundtrip serialization."""
        original = PayloadWrite(path="/test", content="data")
        json_str = original.model_dump_json()
        restored = PayloadWrite.model_validate_json(json_str)
        assert restored == original
...
@pytest.mark.unit
class TestPayloadWriteExtraFieldsRejected:
...
@pytest.mark.unit
class TestPayloadWriteFromAttributes:
    ...
    def test_from_object_with_attributes(self) -> None:
        ...
        payload = PayloadWrite.model_validate(obj)
