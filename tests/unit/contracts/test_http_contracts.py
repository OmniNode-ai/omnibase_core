...
"""Unit tests for ModelHttpRequestContract and ModelHttpResponseContract.
...
@pytest.mark.unit
class TestModelHttpRequestContract:
    """Tests for ModelHttpRequestContract instantiation, defaults, and validation."""

    def test_required_fields_only(self) -> None:
        """Instantiate with only the required fields (method and url)."""
        req = ModelHttpRequestContract(method="GET", url="https://example.com/api")
    ...
    def test_default_headers_is_empty_dict(self) -> None:
        ...
        req = ModelHttpRequestContract(method="GET", url="https://example.com/")
        assert req.headers == {}
    ...
    def test_default_body_is_none(self) -> None:
        ...
        req = ModelHttpRequestContract(
            method="DELETE", url="https://example.com/items/1"
        )
    ...
    def test_extra_fields_forbidden(self) -> None:
        ...
        with pytest.raises(ValidationError):
            ModelHttpRequestContract(  # type: ignore[call-arg]
                method="GET",
    ...
    def test_method_required(self) -> None:
        ...
        with pytest.raises(ValidationError):
            ModelHttpRequestContract(url="https://example.com/")  # type: ignore[call-arg]

    def test_url_required(self) -> None:
        """Omitting url must raise ValidationError."""
        with pytest.raises(ValidationError):
            ModelHttpRequestContract(method="GET")  # type: ignore[call-arg]
    ...
    def test_headers_default_is_not_shared_across_instances(self) -> None:
        ...
        req1 = ModelHttpRequestContract(method="GET", url="https://example.com/")
        req2 = ModelHttpRequestContract(method="POST", url="https://example.com/other")
    ...
    def test_common_http_methods(self) -> None:
        ...
        for method in ("GET", "POST", "PUT", "DELETE", "PATCH", "HEAD", "OPTIONS"):
            req = ModelHttpRequestContract(method=method, url="https://example.com/")
            assert req.method == method


@pytest.mark.unit
class TestModelHttpResponseContract:
    """Tests for ModelHttpResponseContract instantiation, defaults, and validation."""

    def test_required_fields_only(self) -> None:
        """Instantiate with only the required fields (status_code and body)."""
        resp = ModelHttpResponseContract(status_code=200, body=b"OK")
        assert resp.status_code == 200
        assert resp.body == b"OK"

    def test_default_headers_is_empty_dict(self) -> None:
        """Default headers should be an empty dict, not None."""
        resp = ModelHttpResponseContract(status_code=204, body=b"")
        assert resp.headers == {}
    ...
    def test_default_elapsed_seconds_is_none(self) -> None:
        ...
        resp = ModelHttpResponseContract(status_code=200, body=b"content")
    ...
    def test_elapsed_seconds_zero_is_valid(self) -> None:
        ...
        resp = ModelHttpResponseContract(
            status_code=200, body=b"fast", elapsed_seconds=0.0
        )
    ...
    def test_elapsed_seconds_negative_raises_validation_error(self) -> None:
        ...
        with pytest.raises(ValidationError):
            ModelHttpResponseContract(
                status_code=200, body=b"data", elapsed_seconds=-1.0
            )

    def test_extra_fields_forbidden(self) -> None:
        """Extra fields must raise ValidationError (extra='forbid')."""
        with pytest.raises(ValidationError):
            ModelHttpResponseContract(  # type: ignore[call-arg]
                status_code=200,
                body=b"data",
                unknown_field="value",
            )
    ...
    def test_frozen_model_is_immutable(self) -> None:
        ...
        We verify immutability by confirming the model config reports frozen=True.
        """
        resp = ModelHttpResponseContract(status_code=200, body=b"data")
    ...
    def test_status_code_required(self) -> None:
        ...
        with pytest.raises(ValidationError):
            ModelHttpResponseContract(body=b"data")  # type: ignore[call-arg]

    def test_body_required(self) -> None:
        """Omitting body must raise ValidationError."""
        with pytest.raises(ValidationError):
            ModelHttpResponseContract(status_code=200)  # type: ignore[call-arg]

    def test_headers_default_is_not_shared_across_instances(self) -> None:
        """Each instance must get its own default headers dict (no mutable default aliasing)."""
        resp1 = ModelHttpResponseContract(status_code=200, body=b"a")
        resp2 = ModelHttpResponseContract(status_code=404, body=b"not found")
    ...
    def test_various_status_codes(self) -> None:
        ...
        for code in (200, 201, 204, 301, 400, 401, 403, 404, 500, 503):
            resp = ModelHttpResponseContract(status_code=code, body=b"")
...
class TestHttpContractImport:
    ...
    def test_http_request_contract_importable_from_models_contracts(self) -> None:
        """ModelHttpRequestContract must be importable from omnibase_core.models.contracts."""
        from omnibase_core.models.contracts import ModelHttpRequestContract as _Req

        assert _Req is not None

    def test_http_response_contract_importable_from_models_contracts(self) -> None:
        """ModelHttpResponseContract must be importable from omnibase_core.models.contracts."""
        from omnibase_core.models.contracts import ModelHttpResponseContract as _Resp
