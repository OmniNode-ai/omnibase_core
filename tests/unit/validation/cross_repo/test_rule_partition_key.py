...
class TestRulePartitionKey:
    """Tests for RulePartitionKey rule."""
...
"""
    ...
    def test_handles_file_not_under_root_directory(
        ...
        """Test graceful handling when file_path is not under root_directory.

        This can happen with symlinks or when validating files from different
        mount points. The rule should fall back to absolute paths for fingerprints.
        """
        # Create a topic config in a DIFFERENT temp directory than root_directory
        other_tmp = tmp_path / "other"
        other_tmp.mkdir()
        topic_config = other_tmp / "topic_config.py"
        topic_config.write_text(
            """\
class ModelEventTopicConfig:
    topic_name = "events"
...
"""
        ...
        # Use a root_directory that does NOT contain the topic_config file
        ...
        # Should not raise ValueError - should gracefully fall back to absolute path
        issues = rule.validate(file_imports, "test_repo", unrelated_root)

        # Should still detect the missing partition_key
        assert len(issues) == 1
        assert issues[0].code == "MISSING_PARTITION_KEY"
        # Fingerprint should still be generated (using absolute path as fallback)
        assert issues[0].context is not None
        assert issues[0].context.get("fingerprint") is not None
