...
def _hermetic_path(gh_resolver_stub_dir: Path) -> str:
    """Build the resolver PATH from the stub plus only the tools it shells out to.

    The ``gh`` stub must shadow any real ``gh``; ``jq`` and ``bash`` are located
    once so the resolver runs the same way on a developer host and a CI runner.
    """
    tool_dirs = sorted(
        {
            str(Path(found).parent)
            for found in (shutil.which("jq"), shutil.which("bash"))
            if found is not None
        }
    )
    return os.pathsep.join([str(gh_resolver_stub_dir), *tool_dirs, "/usr/bin", "/bin"])
...
def _run_live_pre_check(
    *,
    ...
    env = {
        "PATH": _hermetic_path(gh_resolver_stub_dir),
        "GH_TOKEN": "stub-token",
