...
from omnibase_core.models.llm.model_conversation_context import ModelConversationContext
...
class ModelUserPreferences(BaseModel):
    ...
    preferred_response_length: str = Field(
        default="medium",
        description="Preferred response length (short, medium, long)",
    )
    ...
    preferred_model: str | None = Field(
        default=None,
        description="Preferred LLM model if specified",
    )
...
class ModelConversationSession(ModelOnexInputState):
    ...
    status: EnumConversationStatus = Field(
        default=EnumConversationStatus.INITIALIZED,
