...
class ModelShutdownHandler(BaseModel):
    ...
    priority: EnumShutdownPriority = Field(
        EnumShutdownPriority.NORMAL,
        description="Handler execution priority",
    )
    timeout: float = Field(15.0, description="Handler execution timeout in seconds")
    required: bool = Field(
        True,
        description="Whether handler must complete successfully",
    )

    # Execution tracking
    registered_at: datetime = Field(
        default_factory=datetime.now,
        description="When handler was registered",
    )
    started_at: datetime | None = Field(
        None,
        description="When handler execution started",
    )
    completed_at: datetime | None = Field(
        None,
        description="When handler execution completed",
    )
    ...
    # Metadata
    description: str | None = Field(None, description="Handler description")
...
class ModelShutdownConfiguration(BaseModel):
    ...
    handle_sigusr1: bool = Field(
        True,
        description="Handle SIGUSR1 signal for graceful restart",
    )

    # Shutdown behavior
    force_shutdown_after_timeout: bool = Field(
        True,
        description="Force shutdown after timeout",
    )
    wait_for_handlers: bool = Field(
        True,
        description="Wait for all handlers to complete",
    )
    log_handler_progress: bool = Field(
        True,
        description="Log individual handler progress",
    )

    # Health check integration
    update_health_check: bool = Field(
        True,
        description="Update health check during shutdown",
    )
    drain_requests_timeout: float = Field(
        30.0,
        description="Time to wait for request draining",
    )
...
class ModelShutdownStatus(BaseModel):
    ...
    shutdown_completed: bool = Field(
        False,
        description="Whether shutdown has completed",
    )
    current_phase: EnumShutdownPhase = Field(
        EnumShutdownPhase.RUNNING,
        description="Current shutdown phase",
    )

    # Timing
    shutdown_started_at: datetime | None = Field(
        None,
        description="When shutdown started",
    )
    shutdown_completed_at: datetime | None = Field(
        None,
        description="When shutdown completed",
    )
    ...
    triggering_signal: str | None = Field(
        None,
        description="Signal that triggered shutdown",
    )
    ...
    accepting_requests: bool = Field(
        True,
        description="Whether system is accepting new requests",
    )
    ready_for_termination: bool = Field(
        False,
        description="Whether system is ready to terminate",
    )
