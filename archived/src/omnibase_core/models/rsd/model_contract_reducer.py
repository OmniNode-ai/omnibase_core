...
class ModelStreamingConfig(BaseModel):
    ...
    @classmethod
    def validate_window_overlap(cls, v, info):
        """Validate window overlap is less than window size."""
        if info.data and "window_size" in info.data and v >= info.data["window_size"]:
...
class ModelContractReducer(ModelContractBase):
    ...
    node_type: EnumNodeType = Field(
        default=EnumNodeType.REDUCER, description="Node type must be REDUCER"
    )
    ...
    @field_validator("reduction_operations")
    @classmethod
