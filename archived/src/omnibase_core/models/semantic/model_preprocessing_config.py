...
from omnibase_core.models.semantic.model_chunk_strategy import ModelChunkStrategy
from omnibase_core.models.semantic.model_language_detection_mode import (
    ModelLanguageDetectionMode,
)
from omnibase_core.models.semantic.model_overlap_strategy import ModelOverlapStrategy
...
class ModelPreprocessingConfig(BaseModel):
    ...
    forced_language: str | None = Field(
        None,
    ...
    sentence_splitting: bool = Field(
        default=True,
        description="Enable language-aware sentence boundary detection",
    )
    ...
    preserve_metadata: bool = Field(
        default=True,
        description="Preserve document metadata in chunks",
    )
    ...
    def validate_overlap_size(cls, v: int, info: ValidationInfo) -> int:
        ...
        if v >= chunk_size:
            msg = f"Overlap size ({v}) must be smaller than chunk size ({chunk_size})"
            raise ValueError(
                msg,
            )
        return v
    ...
    def validate_max_chunk_size(cls, v: int, info: ValidationInfo) -> int:
        ...
        if v <= min_chunk_size:
            msg = f"Max chunk size ({v}) must be larger than min chunk size ({min_chunk_size})"
            raise ValueError(
                msg,
            )

        if v < chunk_size:
            msg = f"Max chunk size ({v}) should be larger than target chunk size ({chunk_size})"
            raise ValueError(
                msg,
            )
    ...
    @classmethod
    def validate_stride(cls, v: int | None, info: ValidationInfo) -> int | None:
        ...
        if overlap_strategy == ModelOverlapStrategy.STRIDE_BASED and v is None:
            msg = "Stride must be specified when using stride-based overlap strategy"
            raise ValueError(
                msg,
            )
        ...
        if v is not None:
            chunk_size = data.get("chunk_size", 512)
            if v >= chunk_size:
                msg = f"Stride ({v}) must be smaller than chunk size ({chunk_size})"
                raise ValueError(
                    msg,
                )
    ...
    def validate_forced_language(
        ...
        if language_detection == ModelLanguageDetectionMode.FORCED and v is None:
            msg = "Forced language must be specified when using forced language detection mode"
            raise ValueError(
                msg,
            )
