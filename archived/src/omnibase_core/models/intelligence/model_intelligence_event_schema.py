...
from omnibase_core.models.intelligence.model_event_payload_data import (
...
class ModelLearningWorkflowState(BaseModel):
    ...
    context_data: ModelWorkflowContextData = Field(
        default_factory=lambda: ModelWorkflowContextData(workflow_name="default"),
...
class ModelIntelligenceEventBatch(BaseModel):
    ...
    batch_metadata: ModelBatchProcessingMetadata = Field(
        default_factory=lambda: ModelBatchProcessingMetadata(
            batch_size=1, batch_sequence_number=0, processing_strategy="sequential"
        ),
    ...
    processing_hints: ModelBatchProcessingMetadata = Field(
        default_factory=lambda: ModelBatchProcessingMetadata(
            batch_size=1, batch_sequence_number=0, processing_strategy="adaptive"
        ),
...
class ModelIntelligenceCoordinationSchema(BaseModel):
    ...
    operation_context: ModelOperationContext = Field(
        default_factory=lambda: ModelOperationContext(
            operation_name="default-operation", operation_type="coordination"
        ),
    ...
    success_criteria: ModelSuccessCriteria = Field(
        default_factory=lambda: ModelSuccessCriteria(
            criteria_name="completion_rate",
