...
def fix_id_field_simple(content: str, field_name: str) -> str:
    ...
    pattern = f"(\\s+{field_name}:\\s+)str(\\s*=\\s*Field\\()"
    replacement = "\\1UUID\\2"
...
def fix_id_field_with_uuid_factory(content: str) -> str:
    ...
    pattern = "default_factory=lambda:\\s*str\\(uuid4\\(\\)\\)"
    replacement = "default_factory=uuid4"
    return re.sub(pattern, replacement, content)


def fix_version_field_simple(
    content: str, field_name: str, default_version: ModelSemVer | str = "1.0.0"
) -> str:
    """Fix version field with simple string default"""
    pattern = (
        f'({field_name}:\\s+)str(\\s*=\\s*Field\\(\\s*default=)"{default_version}"'
    )
    parts = default_version.split(".")
    major, minor, patch = (parts[0], parts[1], parts[2] if len(parts) > 2 else "0")
    replacement = f"\\1ModelSemVer\\2_factory=lambda: ModelSemVer(major={major}, minor={minor}, patch={patch})"
    return re.sub(pattern, replacement, content)
    ...
    if has_uuid_import:
        for i, line in enumerate(lines):
            if "from uuid import" in line and "UUID" not in line:
                ...
                "from uuid import" in line and "uuid4" not in line and ("UUID" in line)
            ):
    ...
    try:
        ...
        for field_name, target_type in field_fixes.items():
            if target_type == "UUID":
                ...
                pattern = f"(\\s+{field_name}:\\s+)str(\\s*=\\s*Field\\()"
                content = re.sub(pattern, "\\1ModelSemVer\\2", content)
                needs_semver = True
        if needs_uuid:
...
def main():
    ...
    files_to_fix = {
        ...
        base / "models/security/model_permission_scope.py": {"scope_id": "UUID"},
        base / "models/security/model_detection_pattern.py": {"pattern_id": "UUID"},
        base / "models/core/model_system_data.py": {"system_id": "UUID"},
        base / "models/core/model_instance_metadata.py": {"deployment_id": "UUID"},
        base
