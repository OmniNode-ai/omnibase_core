...
def apply_regex_replacement(filepath: Path, patterns: List[Tuple[str, str]], desc: str) -> int:
...
def main():
    ...
    for file_rel in log_context_files:
        ...
        if content != original:
            ...
            if 'from uuid import UUID' not in content:
                content = re.sub(
                    r'(from uuid import [^\n]+)',
                    r'\1, UUID' if 'from uuid import' in content else 'from uuid import UUID\n\1',
                    content
                )
            with open(filepath, 'w', encoding='utf-8') as f:
                f.write(content)
    ...
    event_files = [
        ...
        ("models/discovery/model_introspection_response_event.py", [(143, "node_id"), (183, "node_id"), (227, "node_id")]),
    ]
    ...
    uuid_none_files = [
        ("infrastructure/node_effect.py", [(404, "ModelTransaction", "transaction_id"), (451, "operation_id", "operation_id")]),
    ]
