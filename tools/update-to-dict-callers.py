        ...
        for line_num, line_content, var_context in callers:
            ...
            if should_update_call(file_path, line_content, var_context):
                ...
                if old_line != new_line:
                    ...
                    updates_made += 1
                    print(f"    Line {line_num}: {var_context}.to_dict() → {var_context}.model_dump(exclude_none=True)")
...
def main():
    ...
    for file_path in sorted(all_files):
        ...
        for line_num, line_content, var_context in callers:
            ...
            print(f"  {status} Line {line_num}: {var_context}.to_dict() - {line_content[:60]}{'...' if len(line_content) > 60 else ''}")
    ...
    print(f"\n" + "=" * 60)
    print(f"🎯 Summary:")
    print(f"  📊 Total files processed: {len([f for f in all_files if find_to_dict_callers(f)])}")
