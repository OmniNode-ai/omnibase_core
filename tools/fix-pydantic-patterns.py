...
class PydanticPatternFixer:
    ...
    def __init__(self, dry_run: bool = True):
        ...
        self.pattern_fixes = [
            ...
            (r'\.dict\(\s*exclude_unset\s*=\s*True\s*\)', '.model_dump(exclude_unset=True)'),
                    ...
                    if re.search(pattern, line):
                        ...
                        if new_line != line:
                            if self._is_likely_pydantic_line(line, file_path):
                                changes.append(f"Line {line_num}: {pattern} -> {replacement}")
                                modified_content = modified_content.replace(original_line, new_line, 1)
                                fixes_in_file += 1
                                line = new_line  # In case multiple patterns on same line
                                break
    ...
    def fix_project(self, src_dir: Path, specific_file: Path = None) -> Dict:
        ...
        for py_file in python_files:
            ...
            if fixes_count > 0:
                relative_path = py_file.relative_to(src_dir.parent) if not specific_file else py_file
        ...
        print(f"\n📊 PATTERN FIX SUMMARY")
        print("=" * 60)
        print(f"Mode: {'DRY RUN (no changes made)' if self.dry_run else 'FIXES APPLIED'}")
