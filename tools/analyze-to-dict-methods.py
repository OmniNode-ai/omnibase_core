...
class ToDigtMethodAnalyzer(ast.NodeVisitor):
    ...
    def _get_return_analysis(self, stmt) -> List[str]:
        ...
        if isinstance(stmt.value, ast.Call):
            if (isinstance(stmt.value.func, ast.Attribute) and 
                stmt.value.func.attr == "model_dump"):
                return ["model_dump_call"]
            elif (isinstance(stmt.value.func, ast.Attribute) and
                  stmt.value.func.attr == "dict"):
                return ["dict_call"]
    ...
    def _categorize_method(self, lines: List[str], node) -> str:
        ...
        if ("dict_construction" in lines or 
            "literal_dict" in lines or
            len(node.body) > 3):
...
def main():
    ...
    complex_methods = (categories.get("complex_custom", []) + 
                      categories.get("complex_with_model_dump", []))
    if complex_methods:
