...
old contract-driven node specs, and research reports were moved to the umbrella
`omni_home` archive/design trees so they are not mistaken for current Core
architecture.
...
The removed historical/design material now lives under `omni_home`:

- `docs/archive/omnibase-core/`
- `docs/design/omnibase-core/`

Promote content back into Core only by extracting current truth into a stable
architecture, reference, decision, or migration document.
